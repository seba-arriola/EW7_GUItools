 ...
 *
 *    $Id: fir_test.c,v 1.1 2000/02/14 17:27:23 lucky Exp $
 *
 *    Revision history:
 *     $Log: fir_test.c,v $
...
main (int argc, char **argv)
{
  ...
  {
    logit("e", "Fir terminating normally for test mode\n");
    exit (EW_SUCCESS);
  }
  ...
  /* Attach to Input shared memory ring 
         *******************************************/
  test_tport_attach (&regionIn, Fir.firEWH.ringInKey);
  ...
  if (Fir.firEWH.ringOutKey == Fir.firEWH.ringInKey)
    ...
    test_tport_attach (&(Fir.regionOut), Fir.firEWH.ringOutKey);
 ...
 /*    Specify logos of incoming waveforms                             */
  ...
  while (test_tport_getmsg (&regionIn, &logoWave, 1, &logoMsg,
  ...
  initqueue (&(Fir.MsgQueue), QUEUE_SIZE, inBufLen);
                          ...
                          &tidFir) == -1)
  {
    ...
    test_tport_detach (&regionIn);
    test_tport_detach (&(Fir.regionOut));
    exit (EW_FAILURE);
  }
...
/*--------------------- setup done; start main loop -------------------------*/
  ...
  while (test_tport_getflag (&regionIn) != TERMINATE)
  {
    /* send fir' heartbeat */
    ...
    ret = test_tport_getmsg (&regionIn, &logoWave, 1, &logoMsg,
    ...
    if ((ret = matchSCN (WaveHeader, &Fir )) < -1 )
    {
      logit ("et", "Fir: Call to matchSCN failed; exitting.\n");
      exit (EW_FAILURE);
    }
    else if ( ret == -1 )
      /* Not an SCN we want */
