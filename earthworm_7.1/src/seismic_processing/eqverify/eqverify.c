 ...
 *
 *    $Id: eqverify.c,v 1.4 2004/07/01 22:45:01 dietz Exp $
 *
 *    Revision history:
 *     $Log: eqverify.c,v $
 ...
 *************************************************/
   eqwt  = 0;
   eqwt += test_slopevsmag();   /* eqmeas test */
   eqwt += test_freefitrms();   /* eqmeas test */
   eqwt += test_codawt();       /* eqmeas test */
   eqwt += test_pgroup();       /* experimental arrival-time test on glitch-groups; LDD*/
    rc   = test_ptimes();       /* Oppenheimer arrival-time test; done for comparison  */
...
/*   else                    i = 0; */
   ...
   if     ( meddt < 0.05 ) i = BadThresh; /* new test is somewhat gentler          */
 ...
 *****************************************************************/
   if( percentglitch >= 0.50    ||
       nP - nglitch  <  mintolocate )  i = BadThresh;
 ...
 ***************************/
 ...
 /*1*/      else if( k_its( "MyModuleId" ) )
  ...
  /*6*/     else if( k_its("test_pgroup") )
         ...
         /* Optional command: reset event weight threshold for
            declaring an event a noise event
          ****************************************************/
            else if( k_its("EventThreshold") )
            {
                BadThresh = k_int();
            }
