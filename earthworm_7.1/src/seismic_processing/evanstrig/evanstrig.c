 ...
 *
 *    $Id: evanstrig.c,v 1.7 2007/03/28 18:31:06 paulf Exp $
 *
 *    Revision history:
 *     $Log: evanstrig.c,v $
...
void Interpolate( char *, int, long  );
...
int main( int argc, char **argv )
{
   ...
   long         *trlong;           /* Long pointer to trace data   */
...
/* Allocate the input message buffer & initialize pointers.
   ...
   *************************************************************/
   buflength = MAX_TRACEBUF_SIZ*2 + sizeof(long)*(MaxGap-1);
   ...
   trlong  = (long *)  (trbuf + sizeof(TRACE2_HEADER));
   ...
   ************************************/
   ...
   /* Get a tracebuf message from transport region
      ********************************************/
      ...
      {
         sprintf( errmsg, "Retrieved msg too big (%d) for buffer\n",
                  length );
   ...
   /* If the samples are shorts, make them longs.
      ...
      ******************************************/
      ...
      {
         int i;
         for ( i = trhd->nsamp - 1; i > -1; i-- )
            trlong[i] = (long) trshort[i];
         trhd->datatype[1] = '4';
      }
   ...
   /* Announce large sample gaps
      **************************/
      ...
      {
         sprintf( errmsg,
                 "Found %4d sample gap; trigger restart on %s %s %s %s\n",
                  GapSize, sta->sta, sta->chan, sta->net, sta->loc );
...
int evanstrig_dotrig( STATION *sta, char *msg, int restart )
{
   ...
   long         *data    = (long *) &msg[sizeof(TRACE_HEADER)];
