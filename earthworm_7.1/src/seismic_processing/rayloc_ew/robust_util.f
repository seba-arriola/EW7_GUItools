...
c
      include 'locprm.inc'
      character*8 phcd1(maxtt),ophase, ph
c	integer stdin, stdout
      real*4 tt1(maxtt),dtdd1(maxtt),dtdh1(maxtt),dddp1(maxtt),usrc(2),
     1 ellip, elcor
      double precision a,b,c,x,s,avdr
c   Physical constants.
      common /circle/ pi,rad,deg,radius
...
c
c Initialize the depth
c
2	if (odep .le. 0.0) odep = 1.0
	zs = odep
      call depset(zs,usrc)
c
...
c
c Find the computed traveltimes and partials for phases other than P and S
c
	do 111 i = 1, n
		ph = phcd1(i)
		ilen = ntrbl(phase)
		if(phase(1:ilen) .eq. ph(1:ilen)) then
			travt = tt1(i)
			dtdd = dtdd1(i)
			dtdh = dtdh1(i)
			ophase = phase
...
100	continue
c
c This was the original call to get the ellipticity and elevation correction,
c   which is replaced by subroutine used by Engdahl.  Correction for the
c   elevation was taken out of subroutine datum
c     call datum(elev,dtdd,olat,delta,azim*deg,tcor,slatc)
c
	azimuth = azim*deg
	tcor=ellip(phase,delta,zs,glat,azimuth)
	if(dtdd.gt.dtds) dtdd=dtds
      elcor=(elev/5.57)*sqrt (1.0-(5.57*dtdd/(radius*rad))**2)
      res=trvtim-osec-travt-(tcor+elcor)
c
c
c	write(merr,333)j,sta,ophase,ph,pflg,delta,azimuth,
c	2				trvtim,res,dtdd,dtdh
c333	format('nclose: ',i5,a8,1x,a8,1x,a8,1x,a1,1x,f6.2,1x,f5.1,1x,
c    1		f8.2,1x,f6.2,1x,f8.4,1x,f8.4)
c
c   All done.  Push data back out to station parameter storage.
c
5     call drdwr(2,j,1,13)
3     continue
c
...
c   by E. R. Engdahl.
c
      double precision a,b,c,x,s,avdr,eps
c     double fac
c   Physical constants.
      common /circle/ pi,rad,deg,radius
c   Storage for the normal equations.
...
c
      include 'locprm.inc'
      character*8 ophase, ph,phcd1(maxtt)
c	integer stdin, stdout
      real*4 tt1(maxtt),dtdd1(maxtt),dtdh1(maxtt),dddp1(maxtt),usrc(2),
     1 ellip, elcor
	common /unit/ min, mout, merr
...
c      derivatives
c
      if (odep .le. 0.0) odep = 1.0
	zs = odep
      call depset(zs,usrc)
c
c The first time that ellip is called, it initializes the routine and then
c   tcor is calculated on subsequent calls.  See further down in the
c   subroutine.
c
...
c   Apply the DRES commands.
     ...
     1   dellim(2,k)) then
      ...
      enddo
c
6     call trtm(delta,maxtt,n,tt1,dtdd1,dtdh1,dddp1,phcd1)
c
c This part of the code is stupid.  Since some of the phases read in might not
c  be identified by branch (e.g PKPdf), we find the first phase that is closest
c  to read in phase.  For example, if the phase read in is "P", but the first
c  phase in time is "Pn" than the phase is rename to "Pn".
c
	do 111 i = 1, n
		ph = phcd1(i)
		ilen = ntrbl(phase)
...
c
	pflg = ' '
c	write(merr,334)sta,phase
c334	format('setup: ','Sta= ',a5,' no phase computed for ',a8)
	go to 8
c
100	continue
c
c Calculate the ellipticity correction for different phases.
c
	azimuth = azim*deg
	tcor=ellip(phase,delta,zs,glat,azimuth)
      if(dtdd.gt.dtds) dtdd=dtds
...
c	write(merr,333)j,navsum,sta,ophase,ph,pflg,delta,azimuth,
c	2				trvtim,res,dtdd,dtdh
c333	format('setup: ',i5,i5,1x,a8,1x,a8,1x,a8,1x,a1,1x,f6.2,1x,f5.1,1x,
c    1		f8.2,1x,f6.2,1x,f8.4,1x,f8.4)
c
...
c
      include 'locprm.inc'
      character*8 phcd1(maxtt),ph, ophase
c	integer stdout,stdin
      real*4 tt1(maxtt),dtdd1(maxtt),dtdh1(maxtt),dddp1(maxtt),usrc(2),
     1 ellip, elcor
c   Physical constants.
...
c
      call trtm(delta,maxtt,n,tt1,dtdd1,dtdh1,dddp1,phcd1)
c
c This part of the code is tricky.  Since some of the phases read in might not
c  be identified by branch (e.g PKPdf), we find the first phase that is closest
c  to read in phase.  For example, if the phase read in is "P", but the first
c  phase in time is "Pn" than the phase is rename to "Pn".
c
...
c
	azimuth = azim*deg
      if(dtdd.gt.dtds) dtdd=dtds
      elcor=(elev/5.57)*sqrt (1.0-(5.57*dtdd/(radius*rad))**2)
	tcor=ellip(phase,delta,zs,glat,azim)
      res=trvtim-osec-travt-(tcor+elcor)
c
c	write(merr,333)j,sta,ophase,ph,pflg,delta,azimuth,
c	2				trvtim,res,dtdd,dtdh
c333	format('stats: ',i5,a8,1x,a8,1x,a8,1x,a1,1x,f6.2,1x,f5.1,1x,
c    1		f8.2,1x,f6.2,1x,f8.4,1x,f8.4)
c
