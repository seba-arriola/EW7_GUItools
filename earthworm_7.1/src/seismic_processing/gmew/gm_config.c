 ...
 *
 *    $Id: gm_config.c,v 1.9 2006/03/15 14:21:54 paulf Exp $
 *
 *    Revision history:
 *     $Log: gm_config.c,v $
 ...
 *            -1 on failure
 */
int Configure( GMPARAMS *pgmParams, int argc, char **argv, EVENT *pEvt)
{
  ...
  case GM_TS_WS:
    ...
    {   /* Set the default traceSource to wave_servers listed in *
      ...
      if ( (pgmParams->pWSV->serverFile = strdup(servDir)) == NULL)
      {
  ...
  {    /* Initialize the station list */
    ...
    if (pgmParams->numSCNLPar > 0)
      qsort(pgmParams->pSCNLPar, sizeof(SCNLPAR), pgmParams->numSCNLPar,
            CompareSCNLPARs);
...
*******************************/
      ...
      else if (k_its("staLoc") )
      {
        if ( (str = k_str()) )
        {
          if (k_its("File") )
          {
            if ( (str = k_str()) )
            {
              if ( (pgmParams->staLocFile = strdup(str)) == NULL)
              {
            ...
            if (k_its("waveServer"))
            {
              ...
              if ( (str = k_str()) )
              {
                ...
                if (k_its("File"))
                {
                  if ( (str = k_str()) )
                  {
                    if ( (pgmParams->pWSV->serverFile = strdup(str)) == NULL)
                    {
                      logit("e", 
      ...
      else if (k_its("respSource") )
      {
        if ( (str = k_str()) )
        {
          if (k_its("File") )
          {
            if ( (str = k_str()) )
            {
              if ( (pgmParams->respDir = strdup(str)) == NULL)
              {
              ...
              if ( (str = k_str()) )
              {
                if ( (pgmParams->respNameFormat = strdup(str)) == NULL)
                {
                  logit("e", 
        ...
        if (pgmParams->saveTrace == GM_UNDEF)
        {
          if ( (str = k_str()) )
          {
            ...
            else if (k_its("SAC") )
            {
              ...
              if ( (str = k_str()) )
              {
                if ( (pgmParams->sacOutDir = strdup(str)) == (char *)NULL)
                {
                ...
                if ( (str = k_str()) )
                {
                  if ( (pgmParams->saveDirFormat = strdup(str)) == 
                       (char *)NULL)
                  {
                  ...
                  if ( (str = k_str()) )
                  {
                    if ( (pgmParams->saveNameFormat = strdup(str)) == NULL)
                    {
                      logit("e", 
      ...
      else if (k_its("EWDBaccess") )
      {
        if ( (str = k_str()) )
        {
          ...
          if ( (pgmParams->pDBaccess->user = strdup(str)) == NULL)
          {
          ...
          if ( (str = k_str()) )
          {
            if ( (pgmParams->pDBaccess->pwd = strdup(str)) == NULL)
            {
            ...
            if (  (str = k_str()) )
            {
              if ( (pgmParams->pDBaccess->service = strdup(str)) == NULL)
              {
	...
	if ( (str = k_str()) )
        {
          if ( (pgmParams->XMLDir = strdup(str)) == NULL)
          {
	...
	if ( (str = k_str()) )
        {
          if ( ( pgmParams->TempDir = strdup(str)) == NULL)
          {
	...
	if ( (str = k_str()) )
        {
          if ( ( pgmParams->MappingFile = strdup(str)) == NULL)
          {
