 ...
 *
 *    $Id: lm_sac.c,v 1.14 2006/03/10 13:03:28 paulf Exp $
 *
 *    Revision history:
 *     $Log: lm_sac.c,v $
...
static int readSACHdr(FILE *fp)
{
  ...
  for (i = 0; i < NUM_FLOAT; i++)
    SwapLong( (long *) &(psh->SACfloat[i]));
  for (i = 0; i < MAXINT; i++)
    SwapLong( (long *) &(psh->SACint[i]));
#endif
                    ...
                    LMPARAMS *plmParams)
{
  ...
  pTm = gmtime( &ltime );
  gSAChd.nzyear = (long)pTm->tm_year + 1900l;
  gSAChd.nzjday = (long)pTm->tm_yday + (long)1; /* julian day, 0 - 365 */
  gSAChd.nzhour = (long)pTm->tm_hour;
  gSAChd.nzmin  = (long)pTm->tm_min;
  gSAChd.nzsec  = (long)pTm->tm_sec;
  gSAChd.nzmsec = (long)( (pEvt->origin_time - (long)pEvt->origin_time) * 
                          1000.0);
