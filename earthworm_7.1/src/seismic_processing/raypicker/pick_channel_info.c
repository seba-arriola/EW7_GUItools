 ...
 *
 *    $Id: pick_channel_info.c,v 1.1 2006/01/30 19:29:30 friberg Exp $
 *
 *    Revision history:
 ...
 *                                                                      *
 ************************************************************************/
int SetChannelSampleRate(double traceSampleRate, PICK_CHANNEL_INFO *channelinfo)
{
    double tolerance;
	int    retc = EW_SUCCESS;
    int    p;
    ...
    else
    {
      ...
      {
          reportError(WD_WARNING_ERROR, NORESULT, "Sampling rate [%lf] does not match previous [%lf], rejecting\n", 
                            traceSampleRate, channelinfo->sampleRate);
          retc = EW_WARNING;
      }
    }
    return retc;
 ...
 *                                                                      *
 ************************************************************************/
static int SetInitialValues(SCNL *scnl)
{  
 ...
 ************************************************************************/
static int STASymmetryEnvelope(SCNL *scnl)
{
 ...
 *                                                                      *
 ************************************************************************/
int ProcessChannelMessage(SCNL *Scnl, RParams rParams, EWParameters EwParams)
{
    PICK_CHANNEL_INFO   *channel = Scnl->pchanInfo; /* pointer to this channel's data              */
    PICK_SERIES_DATA    *workPicker;                /* pick series data for this channel           */
...
*/
      ...
      if (channel->state == PKCH_STATE_NEWPICK)
      {
          ...
          if (rParams.Debug >= 2)
          {
              ...
              sprintf(dbg_filename, "%sraypick_%d_pick.txt", getenv("EW_LOG"), channel->pickId);
              if ((g_debugFile = fopen(dbg_filename,"w")) == NULL)
                  logit("e", "Failed to open pick report debug file %s\n", dbg_filename);
              else
              {
                  fprintf(g_debugFile, "Pick %d for <%s:%s:%s:%s>\n", channel->pickId, 
                            Scnl->sta, Scnl->chan, Scnl->net, Scnl->loc);
                  ...
                  fprintf(g_debugFile, "raypick_%d_sta.txt contain short-term averages for filters <0-n>\n(earliest time first in file)\n", 
                                          channel->pickId);
                  fclose(g_debugFile);
                  g_debugFile = NULL;
              }
              ...
              sprintf(dbg_filename, "%sraypick_%d_sta.txt", getenv("EW_LOG"), channel->pickId);
              if ((g_debugFile = fopen(dbg_filename,"w")) == NULL)
