 ...
 *
 *    $Id: ws_clientII.c,v 1.1 2003/01/31 16:08:20 patton Exp $
 *
 *    Revision history:
 *     $Log: ws_clientII.c,v $
		 ...
		 */
{
  ...
  if (timeout_msec == -1)
      ...
      if (ioctl_ret == SOCKET_ERROR)
	{
	  ret = WS_ERR_SOCKET;
	  if (WS_CL_DEBUG) 
	    fprintf(out_file, "wsWaitBinHeader: error %s occurred during change to blocking\n",
		socketGetError_ew());
	  goto Done;
	}
    }
...
Done:
  ...
  if (timeout_msec == -1) 
      ...
      if (ioctl_ret == SOCKET_ERROR)
	{
	  ...
	  if (WS_CL_DEBUG) 
	    fprintf(out_file,"wsWaitBinHeader: error %s occurred during change to non-blocking\n", 
	      socketGetError_ew() );
	  ret = WS_ERR_SOCKET;
	}
    }
...
Done:
  ...
  if (timeout_msec == -1) 
      ...
      if (ioctl_ret == SOCKET_ERROR) 
	{
	  ...
	  if (WS_CL_DEBUG) fprintf(out_file, "wsWaitAScii: error %s occurred during change to non-blocking\n", 
				 socketGetError_ew() );
	  ret = WS_ERR_SOCKET;
	}
    }
