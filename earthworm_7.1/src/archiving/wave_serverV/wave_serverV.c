 ...
 *
 *    $Id: wave_serverV.c,v 1.36 2007/03/28 18:02:34 paulf Exp $
 *
 *    Revision history:
 *     $Log: wave_serverV.c,v $
...
void    LogTank( TANK* );
int     SocketInit( int, char * );
thr_ret SocketServe( void * );
double  get_starttime( char *, unsigned char );
...
***********************/
int            PassiveSocket;     /* Socket descriptor; passive socket     */
...
/* Get process ID for heartbeat messages */
  ...
  *******************************/
      ...
      start of loop over messages ***/
  {
    ...
    if( result < 0 ) /* -1 means empty queue */
    {
    ...
    /* Swap the message to local notation
     *************************************/
    rc = WaveMsg2MakeLocal( (TRACE2_HEADER*)inMsg );  /* recall that inMsg is one "int" past deQMsg */

    if(rc < 0)
    {
      logit("et","WARNING: WaveMsg2MakeLocal() rejected tracebuf.  Discarding: (%s.%s.%s.%s)\n",
        trh.sta, trh.chan, trh.net, trh.loc);
      logit("e", "\t%.2f - %.2f nsamp %d samprate %6.2f endtime %.2f.\n",
        trh.starttime, trh.endtime, trh.nsamp, trh.samprate,
        IndexYoungest(&Tanks[t])->tEnd);
      goto ReleaseMutex;
    }
    ...
    memcpy((void*)&trh, (void*)inMsg, sizeof(TRACE2_HEADER));
    if(!TraceBufIsValid(&Tanks[t],&trh))
    {
	logit("et","WARNING: Tracebuf seems invalid.  Discarding: (%s.%s.%s.%s)\n",
	      trh.sta, trh.chan, trh.net, trh.loc);
	logit("e", "\t%.2f - %.2f nsamp %d samprate %6.2f endtime %.2f.\n",
	      trh.starttime, trh.endtime, trh.nsamp, trh.samprate,
	      IndexYoungest(&Tanks[t])->tEnd);
    ...
    {
      sprintf( Text, "tank %s.%s.%s.%s msg too big (%d > %d).\n",
    ...
    if ( (long) fwrite( (void*)inMsg, Tanks[t].recSize, 1, Tanks[t].tfp ) != 1 )
  ...
  while( !terminate )
  {
    ...
    if( res != GET_OK )
    {
      if( res==GET_TOOBIG )
      {
        if(sayItOnce==0)
        {
          sprintf( Text, "tport msg[%ld] i%d m%d t%d too big. Max is %ld",
    ...
    if(pTempTank)
    {
      t=(pTempTank-Tanks);  /* both are TANK *'s so sizeof(TANK) is
                               already factored in */
    }
    else
    {
...
thr_ret ServerMgr( void *dummy )
{
  ...
  char16 * client_ip;  /* IP address of client from inet_ntoa   */
...
#define ACCEPTABLE_TIME_TO_WAIT_FOR_SERVER_THREAD_TO_START 15
  ...
  ******************************************************************/
  ...
  /*ActiveSocket=(int *) malloc(MaxServerThreads * sizeof(int));*/
  ...
  while(nfiles > 0)   /* While there are command files open */
  {
    while(k_rd())        /* Read next line from active file  */
    {
      ...
      /*12*/    else if( k_its("Tank") )
      {
        ...
        if(str=k_str())	strncpy(Tanks[nTanks].sta, str, TRACE2_STA_LEN);
        else 	{
          logit ("e", "wave_serverV: bad station name\n");
          exit(-1);
        }
        if(str=k_str())	strncpy(Tanks[nTanks].chan, str, TRACE2_CHAN_LEN);
        else 	{
          logit ("e", "wave_serverV: bad component name\n");
          exit(-1);
        }
        if(str=k_str())	strncpy(Tanks[nTanks].net, str, TRACE2_NET_LEN);
        else 	{
          logit ("e", "wave_serverV: bad network name\n");
          exit(-1);
        }
        if(str=k_str())	strncpy(Tanks[nTanks].loc, str, TRACE2_LOC_LEN);
        else 	{
...
{
  logit("t","wave_serverV.c:Version 5.1.27 - 1143479555\n");
  return(0);
}
...
int TraceBufIsValid(TANK * pTSPtr, TRACE2_HEADER * pCurrentRecord)
{
    ...
    /* check that time goes forward between the previous tracebuf and this
       one */
    if(pCurrentRecord->starttime < IndexYoungest(pTSPtr)->tEnd) {
	if (Debug > 1)
