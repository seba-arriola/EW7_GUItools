 ...
 *
 *    $Id: index_util.c,v 1.7 2007/02/26 21:41:33 paulf Exp $
 *
 *    Revision history:
 *     $Log: index_util.c,v $
...
RedundantFilePtr CreateFilePtr(char* FName1, char* FName2)
{
  ...
  {
    fprintf(stderr, "Null pointer passed to CreateFilePtr: FN1 %u FN2 %u\n",
          FName1,FName2);
    return(NULL);
  }
               ...
               int BytesToWrite, char* pBuffer, int FinishedWithFile)
{
  ...
  retVal=fwrite(pBuffer, BytesToWrite, 1 /*Block*/, rfPtr->pCurrentFile);
    ...
    else
    {
      if(tfiBuffer.NumOfTanks <= MAX_TANKS)
      {
        ...
        if(ReadRFile(pTANKList->pTSFile,
                     sizeof(tfiBuffer),BufferSize,pBuffer) != 1)
        {
                  ...
                  int CreateNew)
{
  ...
  if(UseRedundantIndexFiles)
  {
    ...
    TankNameLength=strlen(Filename1);
  ...
  else
  {
    ...
    */
        ...
        CurrentPosition+=pTSPtr->recSize)
    {
      ...
      {
        logit("t","Bad record found in %s \n\tat %ld: start: %f end: %f tank end %f\n",
