 ...
 *
 *    $Id: serve_trace.c,v 1.14 2007/03/28 18:02:34 paulf Exp $
 *
 *    Revision history:
 *     $Log: serve_trace.c,v $
...
int _writeTraceDataRaw( int, TANK*, CLIENT_MSG* );
int _writeTraceDataAscii( int, TANK*, CLIENT_MSG* );

static int SendReqDataRaw( int, TANK*, double, double, double, double,
                           double, double,
                           char*, long int, char* );
static int SendReqDataAscii( int, TANK*, double, double, double, double,
                             double, double, char*,
                             char*, long int, char* );
...
static int BulkSendFill( int, char*, long int );
static int SendHeader( int soc, TANK* t, char* reqid, char* flag, double tS,
		       double tE, int binSize, int flagNewLine);
static int SendStrData( int soc, char* str );
 ...
 *    input: int soc, socket descriptor;
...
int _writeTraceDataRaw( int soc, TANK* t, CLIENT_MSG* msg )
{
  int ret = R_FAIL;
  ...
  {
    logit( "","_writeTraceDataRaw(): ftell[%ld] failed for tank %s\n",
           saveOff, t->tankName );
    goto abort;
  }
  ...
  if ( ( acttS < tE && tS < acttE ) &&
       ( acttS + DBL_FLOAT_EPSILON <= acttE) )
  {
    if ( CopyReqData( t, oS, oE, (char**) &data, &datasize ) == R_FAIL )
      goto abort;
  }
 ...
 skip:
  if ( ret != R_DONE)
  {
    ret = R_SKIP;
    if ( _fDBG & _fSUB )
      logit( "","_writeTraceDataRaw(): skips processing\n" );
  }
 abort:
  if ( ret == R_FAIL )
  {
    logit( "","_writeTraceDataRaw(): failed\n" );
  }
  if ( mutexLocked ) 
    ReleaseSpecificMutex(&(t->mutex));
...
int _writeTraceDataAscii( int soc, TANK* t, CLIENT_MSG* msg )
{
  int ret = R_FAIL;
  long int oS = -1L;          /* starting offset for CopyReqData() */
  long int oE = -1L;          /* ending offset for CopyReqData() */
                      ...
                      int DirectionFlag)
{
  ...
  *acttp = 0.0;
  ...
  if ( !t || !( t->tfp ) || t->samprate < DBL_FLOAT_EPSILON )
  {
...
static int LocateStartOfTank( TANK* t, long int* offp, double* acttp )
{
  ...
  *acttp = 0.0;
  ...
  if ( !t || !( idx = IndexOldest( t ) ) ) goto abort;

  *offp = idx->offset;
  *acttp = idx->tStart;

  if ( _fDBG & _fPRT )
    logit( "","    LocateStartOfTank():    indxS->off[%ld] indxS->sT[%f]\n",
	   *offp, *acttp );
  ret = R_DONE;
 abort:
  if ( ret != R_DONE )
                        ...
                        char** datap, long int* sizep )
{
  ...
  *sizep = 0;
  ...
  if ( !t || !( tfp = t->tfp ) || !( tsz = t->tankSize ) ) goto abort;
...
int SendReqDataAscii_AndThisTimeWeMeanIt(char * data, int iDataByteLen, 
                                         ...
                                         int iRecordSize, TANK * t, int soc,
  ...
  */
      ...
      pCurrentRecord += iRecordSize) 
  {
    ...
    if ( bFirstBlock && tReqStart > pTH->starttime)
    {
      ...
      if ( _fDBG & _fPRT ) logit( "","SendReqDataAscii_AndThisTimeWeMeanIt(): adj leading iFirstSample to [%ld]\n",iFirstSample );
    }
    ...
    if ((pCurrentRecord + iRecordSize) > (data + iDataByteLen)) 
    {
     ...
     iLastSample -= ((pCurrentRecord + iRecordSize) - (data + iDataByteLen) 
    ...
    if (pTH->endtime > tReqEnd) 
    {
      ...
      if ( _fDBG & _fPRT ) 
        logit( "","SendReqDataAscii_AndThisTimeWeMeanIt(): adj trailing nsamp to [%ld]\n",iLastSample );
    }
    ...
    */
    ...
    for (i=iFirstSample; i<iLastSample; i++)
    {
      ...
      if ( iDataSize == D_SIZE_I2 )
        ival = (long int) *((short int*)&pCurrentRecord[samp_pos]);
      else /* iDataSize == D_DIZE_I4 */
        ival = *((long int*)&pCurrentRecord[samp_pos]);
      sprintf( &blk_buf[ blk_pos ], "%d ", ival );
      blk_pos = strlen( blk_buf );
    }
 ...
 *    input: int soc, socket descriptor supplied by caller
 *           TANK* t, with the assumption t->tfp opened in "br+" mode;
 *           double tS, starting time of reqested period;
 *           double tE, ending time of reqested period;
...
static int SendReqDataAscii( int soc, TANK* t, double tS, double tE,
                             double tL, double tR,
...
/*  
  ...
  long int rec_pos = 0L;
  long int nsamp = 0; /* need long int for possibly numerous fill values * /
  int dsz = 0;
  long int itr = 0;   /* need iterate through nsamp of fill values * /
  long int samp_pos = 0L;
  double sampt = tS;  /* time stamp when the last sample was sent * /
  long int ival = 0;
  char* blk_buf = (char*) NULL;
  long int blk_pos = 0L;
  int BadFirstRecord=0;
  int retVal;
  TRACE2_HEADER* trace_hp;
*/  
...
static int BulkSendFill( int soc, char* fill, long int fill_cnt )
{
  int ret = R_FAIL;
		       ...
		       double tE, int binSize, int flagNewLine)
{
  ...
  if ( !t || !flag ) return R_FAIL;
    
  if (sizeof(int) == D_SIZE_I4)
    sprintf( buffer, "%s %ld %s %s %s %s %s %s ", reqid, t->pin, t->sta, t->chan,
	     t->net, t->loc, flag, t->datatype);
  else  /* sizeof(int) == D_SIZE_I2 */
  ...
  len = strlen(buffer);
  if ( send_ew(soc, buffer, len, 0, SocketTimeoutLength) != (int)len )
  {
...
static int SendReqDataRaw( int soc, TANK* t, double tS, double tE,
                           double tL, double tR,
                           double acttS, double acttE,
                           char* data, long int datasize, char* reqid )
{
