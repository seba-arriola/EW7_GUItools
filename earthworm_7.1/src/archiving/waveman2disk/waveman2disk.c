 ...
 *
 *    $Id: waveman2disk.c,v 1.12 2004/07/28 22:41:20 lombard Exp $
 *
 *    Revision history:
 *     $Log: waveman2disk.c,v $
 *     Revision 1.12  2004/07/28 22:41:20  lombard
 *     Cleaned up a logit call.
 *
 *     Revision 1.11  2004/07/16 20:47:13  lombard
 ...
 *
 *     Revision 1.10  2001/10/04 20:56:57  patton
 *     Made logit changes due to new logit code.
 *     JMP 10/4/2001
 *
 *     Revision 1.9  2001/04/12 04:26:55  lombard
 *     Major cleanup and reformating of code.
 *     Removed checks for TravelTimeout which was never implemented on wave_serverV
 *
 *     Revision 1.8  2001/03/22 21:02:12  cjbryan
 *     deleted all references to EventMod and EventInst as these variables
 *     are now obsolete
 *     added EventSubnet variable to PA_next_ev paramater list as this is used by
 *     CVO in putaway routines
 *
 *     Revision 1.7  2000/08/08 17:17:34  lucky
 *     Lint Cleanup
 *
 *     Revision 1.6  2000/08/01 21:16:09  lucky
 *     Null-terminated EventTime string.
 *
 *     Revision 1.5  2000/07/12 17:20:26  lucky
 *     Increased TIME_CHARS to 11 -- this was necessary to comply with the changes to the
 *     putaway routines, and how they call epochsec17.
 *
 *     Revision 1.4  2000/07/08 20:07:23  lombard
 *     Fixed miising null terminator in YYYYMMDD; corrected declaration of nextc
 *     removed some unused varaibles.
 *
 *     Revision 1.3  2000/05/24 15:59:53  lucky
 *     Fixed the interactive user prompt to be YYYYMMDDHHMMSS (instead of YYYYMMDDHHSS).
 *
 *     Revision 1.2  2000/03/13 21:40:20  lucky
 *     Fixed calls to PA_xxx to reflect the changes in the putaway routines.
 *
...
int main(int argc, char **argv)
{
    ...
    /* file is read.  JMP 10/4/2001 */
    logit_init ("waveman2disk", 0, 1024, 1);

    /* Check command line arguments */
    if (argc != 2) {
	fprintf (stderr, "Usage: waveman2disk <configfile>\n");
	return EW_FAILURE;
    }
  ...
  (void)sigignore(SIGPIPE);
    ...
    logit_init ("waveman2disk", 0, 1024, LogSwitch);
	...
	while (done == FALSE) {
		...
		*/ 
		...
		if (ret != WS_ERR_NONE) {
			...
			logit ("e", 
			       "MaxTraces (%d) exceeded. Some traces not saved\n",
			       MaxTraces); 
			break;
		    case WS_ERR_INPUT:
...
static int WriteEvent (void)
{
	...
	logit ("", "calling PA_next_ev(TraceReq = %u, nTrReq=%u,"
	       "Snppt.eventID = %s,  Snppt.author = %s, Debug = %u\n",
	       TraceReq, nTrReq, EventID, Snppt.author, Debug);
 ...
 */
static int GetInfoFromSnippet(SNIPPET Snppt)
{
    int	i, j;
