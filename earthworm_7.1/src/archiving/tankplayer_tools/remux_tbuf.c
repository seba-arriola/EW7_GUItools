...
main( int argc, char *argv[] )
{
  ...
  *****************/
  ...
  if ( ifp == NULL )
  {
    logit("et", "remux_tbuf: Cannot open demuxed wavefile <%s>\n", argv[1] );
    return(1);
  }
  ofp = fopen( argv[2], "wb" );
  if ( ofp == NULL )
  {
    logit("et", "remux_tbuf: Cannot open output wavefile <%s>\n", argv[2] );
    return(1);
  }
  ...
  if ( tbuf == NULL )
  {
    logit("et", "remux_tbuf: error allocating %d bytes\n", 
  ...
  **************************************************************************/
         ...
         == sizeof( TRACE2_HEADER ) )  
  {
     ...
     ************************/
     if( tbuf[nbuf].size > MAX_TRACEBUF_SIZ ) {
       logit("et",
               "remux_tbuf: msg[%d] overflows internal buffer[%d]\n",
     ...
     if( nbuf >= maxnumbuf )
     {
        ...
        if( tbuf == NULL )
        {
           logit("et", "remux_tbuf: Could not realloc tbuf to %d bytes\n", 
                    maxnumbuf*sizeof(TBUF) );
           goto done;
        }
     }
  ...
  for( ib=0; ib<nbuf; ib++ )
  {
     ...
     if( fread( pkt.msg, sizeof(char), tbuf[ib].size, ifp ) 
                != tbuf[ib].size )
     {
        logit("et", "remux_tbuf: error reading input file at byte %d\n",
                tbuf[ib].offset );
        goto done;
     }      
     if( fwrite( pkt.msg, sizeof(char), tbuf[ib].size, ofp )  
                != tbuf[ib].size )
     {
       logit("et", "remux_tbuf: error writing %d bytes to output file\n",
               tbuf[ib].size );
       goto done;
     }      
  }

  logit("et", "remux_tbuf: finished writing file %s\n", argv[2] );
