 ...
 *    Revision history:                                     *
 *                                                          *
 *     $Log: ewdb_delete_picks_before_time.sql,v $
...
CREATE OR REPLACE FUNCTION Delete_Picks_Before_Time
(IN_tTime            number,
 IN_iMaxRecsToDelete number
) RETURN NUMBER

  /*  Delete_Picks_Before_Time Return codes:
           0 :   Success
...
as
  ...
  Cursor Pick_cursor(Cursor_tTime number) is
    select * from Pick
      where tPhase < Cursor_tTime
      order by tPhase asc;
...
begin
  ...
  for Curr in Pick_cursor(IN_tTime) loop
    Temp_RetCode := Delete_Pick(Curr.idPick);
    if Temp < 0 then
      return(Temp);
    elsif Temp > 0 then
      Temp_RetCode := 1;
...
EXCEPTION
  WHEN OTHERS THEN
    insert into test values('Delete_Picks_Before_Time',IN_tTime,State);
	  return(-1);
END;
