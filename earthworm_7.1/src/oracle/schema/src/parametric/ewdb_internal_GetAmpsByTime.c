 ...
 *
 *    $Id: ewdb_internal_GetAmpsByTime.c,v 1.6 2005/06/27 15:31:36 davidk Exp $
 *
 *    Revision history:
 *     $Log: ewdb_internal_GetAmpsByTime.c,v $
 *     Revision 1.6  2005/06/27 15:31:36  davidk
 ...
 *
 *     Revision 1.2  2003/08/21 00:58:14  davidk
...
static char SQL_STRING[] =
         ...
         "select idPeakAmp, idChan, dPeakAmp1, tPeriod1, tAmp1, "
        " dPeakAmp2, tPeriod2, tAmp2, tStartInterval, tEndInterval "
         "from ALL_PEAKAMPS "
         "where tAmp1 >= :IN_tStart AND tAmp1 <= :IN_tEnd AND iMagType = :IN_iMagType";
...
static EWDB_OCI_SFS SQLParamsBindArray[] = 
{
  ...
  {0,1,20,0,0,OA_DOUBLE,"9tStartInterval"},
  {0,1,20,0,0,OA_DOUBLE,"10tEndInterval"},
...
/* Insertion Struct for GetAmpsByTime statement */
...
/********************************
      FUNCTION PROTOTYPES
********************************/
static int PrepGetAmpsByTimeExec(time_t IN_tStart, time_t IN_tEnd, MAGNITUDE_TYPE IN_iMagType, 
                          EWDB_Cursor * ppCursor);
static int PostGetAmpsByTimeExec(EWDB_PeakAmpStruct * pBuffer, int BufferRecLen);
                                ...
                                int iBufferLen)
{
  ...
  {
    ewdb_base_ErrorReport(hEWDBC, pCursor,"ewdb_internal_GetAmpsByTime(): ewdb_base_SQLExecute",1);
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  } 
  ...
  ****************************************************/
  ...
  {
    ewdb_base_ErrorReport (hEWDBC, hEWDBC,"ewdb_internal_GetAmpsByTime():ewdb_base_SQLCommit",2);
    return (ewdb_base_Disconnect (EWDB_RETURN_FAILURE));
  }
 
  ewdb_base_SetLastOraAPIActionTime();
  if((*pNumAmpsFound = PostGetAmpsByTimeExec(pAmpBuffer,iBufferLen)) 
     == EWDB_RETURN_FAILURE)
  {
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
...
}  /* end ewdb_internal_GetAmpsByTime() */
                                      ...
                                      EWDB_OCIStatementStruct *pSS)
{
  ...
  if(!pLocalBuffer)
  {
    ...
    LastSize=pSS->FieldArray[8].Ind; /* tStartInterval */
    pSS->FieldArray[9].pVal=(void *)(
     (int)(pSS->FieldArray[8].pVal)+(LastSize*iRecordsPerBuffer));
    LastSize=pSS->FieldArray[9].Ind; /* tEndInterval */

    pSS->FieldArray[10].pVal=&Local_tStart;
    pSS->FieldArray[11].pVal=&Local_tEnd;
  ...
  if(!pLocalBuffer)
  {
    logit("","InitGetAmpsByTimeStatement: malloc of pLocalBuffer "
          "failed! Returning.\n");
    return(EWDB_RETURN_FAILURE);
  }

  return(ewdb_base_RequestCursor(szStatement, pSS,0/*don't force rebind*/));
}  /* End InitGetAmpsByTimeStatement() */
...
static int PostGetAmpsByTimeExec(EWDB_PeakAmpStruct * pBuffer, int BufferRecLen)
{
  ...
  if(RowsRetrieved > BufferRecLen)
  {
    ...
    */
    ...
    if(ewdb_base_GetCursorRetCode(pCursor) != EWDB_SQL_ERROR_NO_DATA)
    {
      ewdb_base_ErrorReport(hEWDBC, pCursor,"PostGetGetAmpsByTimeExec:ewdb_base_SQLFetchRows",2);
      return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
    }
    /* else */
