 ...
 *
 *    $Id: ewdb_api_GetMwTimeSeriesByPick.c,v 1.6 2005/06/15 19:03:12 davidk Exp $
 *
 *    Revision history:
 *     $Log: ewdb_api_GetMwTimeSeriesByPick.c,v $
...
static char SQL_STRING[] =
       "select MWCTS.idMwCTS, MWCTS.dTSStart, MWCTS.dTSFreq, MWCTS.iTSLen, "
       "       MWCTS.idMwFilter, MWCTS.idChan, MWCTS.iType, MWCTS.sTS1, MWCTS.sTS2 "
       " from ALL_MWCTS_INFO MWCTS, ALL_MWCHAN_INFO MWCHAN"
       " where MWCHAN.idPick = :IN_idPick "
       "   and (MWCHAN.idSynthTS = MWCTS.idMwCTS OR MWCHAN.idRealTS = MWCTS.idMwCTS) "
       "   and MWCTS.iType = :IN_iType "
...
/* Statement Struct for GetMwTimeSeriesByPick statement */
...
static  EWDBid  Local_idPick, Local_idMwFilter;
...
/********************************
      FUNCTION PROTOTYPES
********************************/
static int PrepGetMwTimeSeriesByPickExec(EWDBid IN_idPick, EWDB_MwTimeSeriesStruct * pMwCTS,
                                         EWDB_Cursor * ppCursor);
static int PostGetMwTimeSeriesByPickExec(EWDB_MwTimeSeriesStruct * pMwCTS);
static int InitGetMwTimeSeriesByPickStatement(char * szStatement, 
                                              EWDB_OCIStatementStruct *pSS);


int ewdb_api_GetMwTimeSeriesByPick(EWDBid IN_idPick, EWDB_MwTimeSeriesStruct * pMwCTS)
/* fill in IN_idPick, and get back the earliest time series linked to that pick.
   Caller must fill in:   IN_idPick, pMwCTS->cTSType, and pMwCTS->idMwFilter.
  ...
  */
{
  ...
  if(!(IN_idPick && pMwCTS))
  {
    logit("et", "ewdb_api_GetMwTimeSeriesByPick():  Error null parameter(s) "
                "passed in! (%u/%u\n",
          IN_idPick, pMwCTS);
    return(EWDB_RETURN_FAILURE);
  }

  if(!(pMwCTS->idMwFilter))
  {
    logit("et", "ewdb_api_GetMwTimeSeriesByPick():  Error null idMwFilter parameter\n");
    return(EWDB_RETURN_FAILURE);
  }
  ...
  if( PrepGetMwTimeSeriesByPickExec(IN_idPick, pMwCTS, &pCursor)
     != EWDB_RETURN_SUCCESS)
  {
    logit("","ewdb_api_GetMwTimeSeriesByPick(): PrepGetMwTimeSeriesByPickExec() failed!\n");
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }

  if( ewdb_base_SQLExecute(pCursor) )
  {
    ewdb_base_ErrorReport(hEWDBC, pCursor,"ewdb_api_GetMwTimeSeriesByPick(): ewdb_base_SQLExecute",1);
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  } 

  if(ewdb_base_SQLCommit(hEWDBC))
  {
    ewdb_base_ErrorReport(hEWDBC, hEWDBC, "ewdb_api_GetMwTimeSeriesByPick(): ewdb_base_SQLCommit",2);
    return(ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
  
  if((rc = PostGetMwTimeSeriesByPickExec(pMwCTS)) 
     == EWDB_RETURN_FAILURE)
  {
    logit("","ewdb_api_GetMwTimeSeriesByPick(): PostGetMwTimeSeriesByPickExec() failed!\n");
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
  ...
  return(rc);
}  /* end ewdb_internal_GetMwTimeSeriesByPick() */
                                              ...
                                              EWDB_OCIStatementStruct *pSS)
{
  ...
  if(!pLocalBuffer)
  {
    ...
    iRecordSize += pSS->FieldArray[8].Ind;/*7sTS2*/
    ...
    {
      logit("et","ERROR! InitGetMwTimeSeriesByPickStatement(): not enough bs(%d): %d needed!\n",
    ...
    pSS->FieldArray[9].pVal=&Local_idPick;
    pSS->FieldArray[10].pVal=&Local_iType;
    pSS->FieldArray[11].pVal=&Local_idMwFilter;

  } /* end if(!pLocalBuffer) */
  
  if(!pLocalBuffer)
    logit("","InitGetMwTimeSeriesByPickStatement: malloc of pLocalBuffer "
          "failed! Returning.\n");


  return(ewdb_base_RequestCursor(szStatement, pSS,0/*don't force rebind*/));
}  /* End InitGetMwTimeSeriesByPickStatement() */
                                         ...
                                         EWDB_Cursor * ppCursor)
{
  ...
  if(InitGetMwTimeSeriesByPickStatement(SQL_STRING, &SSStatement) == EWDB_RETURN_FAILURE)
    return(EWDB_RETURN_FAILURE);

  *ppCursor=SSStatement.pCda;

  return(EWDB_RETURN_SUCCESS);
}  /* End PrepGetMwTimeSeriesByPickExec() */
...
static int PostGetMwTimeSeriesByPickExec(EWDB_MwTimeSeriesStruct * pMwCTS)
{
  ...
  while(!done)
  {
    ...
    if(ewdb_base_SQLFetchRows(pCursor, iRecordsPerBuffer))
    {
      ...
      {
        ewdb_base_ErrorReport(hEWDBC, pCursor,"PostGetMwTimeSeriesByPickExec:ewdb_base_SQLFetchRows",1);
        return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
      }
    }
    ...
    for(; RowsDone < RowsRetrieved; RowsDone++)
    {
      ...
      /* CopyRowFromBuffertoUserBuffer */
      {
        ...
        {
          logit("et","ERROR: PostGetMwTimeSeriesByPickExec(%d) Failure in ewdb_internal_TS_SQLtoC()!  Aborting\n",
        ...
        {
          logit("et","ERROR: PostGetMwTimeSeriesByPickExec(%d) TimeSeries lengths do not match. Given(%d) Parsed(%d)!  Aborting\n",
                pMwCTS[UCurr].idMwCTS);
          ewdb_base_ReleaseCursor(pCursor);
          return(EWDB_RETURN_FAILURE);
        }
...
}  /* End PostGetGetMwTimeSeriesByPickExec() */
