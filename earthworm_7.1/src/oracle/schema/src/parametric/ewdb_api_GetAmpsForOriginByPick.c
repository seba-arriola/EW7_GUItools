 ...
 *
 *    $Id: ewdb_api_GetAmpsForOriginByPick.c,v 1.4 2005/06/27 15:31:36 davidk Exp $
 *
 *    Revision history:
 *     $Log: ewdb_api_GetAmpsForOriginByPick.c,v $
 ...
 *     DB API Cleanup
 *
 *     Revision 1.1  2004/07/19 17:57:18  davidk
 *     Function to retrieve a list of amplitudes for an Origin via origin-pick-amplitude
 *     association.
 *
 *     Revision 1.3  2003/09/16 16:56:55  davidk
 *     General API Cleanup
 *
 *     Revision 1.2  2003/08/21 00:55:49  davidk
...
static char SQL_STRING[] =
         ...
         "       idPick, iMagType, tStartInterval, tEndInterval, sSource, xidExternal "
         " from ALL_AMPS_FOR_ORIGIN_BY_PICK "
         " where idOrigin = :IN_idOrigin "
         "   AND iMagType = :IN_iMagType";
...
static EWDB_OCI_SFS SQLParamsBindArray[] = 
{
  ...
  {0,1,0,0,0,OA_EWDBID, "9idPick"},
  {0,1,0,0,0,OA_INT,    "10iMagType"},
  {0,1,20,0,0,OA_DOUBLE,"11tStartInterval"},
  {0,1,20,0,0,OA_DOUBLE,"12tEndInterval"},
  {0,1,20,0,0,OA_SZ,    "13sSource"},
  {0,1,20,0,0,OA_SZ,    "14xidExternal"},
...
/* Insertion Struct for GetAmpsForOriginByPick statement */
...
/********************************
      FUNCTION PROTOTYPES
********************************/
static int PrepGetAmpsForOriginByPickExec(EWDBid IN_idOrigin, MAGNITUDE_TYPE IN_iMagType, 
                                          EWDB_Cursor * ppCursor);
static int PostGetAmpsForOriginByPickExec(EWDB_PeakAmpStruct * pBuffer, int BufferRecLen);
static int InitGetAmpsForOriginByPickStatement(char * szStatement, 
                                    ...
                                    int iBufferLen)
{
  ...
  if(IN_idOrigin <= 0 || !pAmpBuffer || !pNumItemsFound || !pNumItemsRetrieved)
  {
    logit("","ewdb_api_GetAmpsForOriginByPick(): ERROR!  Invalid parameter(s) passed in: "
             "IN_idOrigin %d, pAmpBuffer %u, pNumItemsFound %u, pNumItemsRetrieved %u\n",
          IN_idOrigin, pAmpBuffer, pNumItemsFound, pNumItemsRetrieved);
  ...
  if(PrepGetAmpsForOriginByPickExec(IN_idOrigin, IN_iMagType, &pCursor)
     != EWDB_RETURN_SUCCESS)
  {
    logit("","ewdb_api_GetAmpsForOriginByPick(): PrepGetAmpsForOriginByPickExec() failed!\n");
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }

  if( ewdb_base_SQLExecute(pCursor) )
  {
    ewdb_base_ErrorReport(hEWDBC, pCursor,"ewdb_api_GetAmpsForOriginByPick(): ewdb_base_SQLExecute",1);
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  } 
  ...
  ****************************************************/
  ...
  {
    ewdb_base_ErrorReport (hEWDBC, hEWDBC,"ewdb_api_GetAmpsForOriginByPick(): ewdb_base_SQLCommit",2);
    return (ewdb_base_Disconnect (EWDB_RETURN_FAILURE));
  }
 
  if((*pNumItemsFound = PostGetAmpsForOriginByPickExec(pAmpBuffer,iBufferLen)) 
     == EWDB_RETURN_FAILURE)
  {
    logit("","ewdb_api_GetAmpsForOriginByPick(): PostGetAmpsForOriginByPickExec() failed!\n");
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
...
}  /* end ewdb_api_GetAmpsForOriginByPick() */
                                               ...
                                               EWDB_OCIStatementStruct *pSS)
{
  ...
  if(!pLocalBuffer)
  {
    ...
    iRecordSize += sizeof(EWDBid); /*9idPick*/
    iRecordSize += sizeof(int);/*10iAmpType*/
    iRecordSize += pSS->FieldArray[10].Ind; /*11tStartInterval*/
    iRecordSize += pSS->FieldArray[11].Ind; /*12tEndInterval*/
    iRecordSize += pSS->FieldArray[12].Ind; /*13szExtSource*/
    iRecordSize += pSS->FieldArray[13].Ind; /*14szExternalAmpID*/
    ...
    LastSize=pSS->FieldArray[2].Ind; /* dPeakAmp1 */
    ...
    LastSize=pSS->FieldArray[5].Ind; /* dPeakAmp2 */
                                          ...
                                          EWDB_Cursor * ppCursor)
{
  ...
  rc = InitGetAmpsForOriginByPickStatement(SQL_STRING, &SSStatement);

  *ppCursor=SSStatement.pCda;

  return(rc);
}  /* End PrepGetAmpsForOriginByPickExec() */
...
static int PostGetAmpsForOriginByPickExec(EWDB_PeakAmpStruct * pBuffer, int BufferRecLen)
{
  ...
  while(!done)
  {
    ...
    if(ewdb_base_SQLFetchRows(pCursor, iRecordsPerBuffer))
    {
      ...
      {
        ewdb_base_ErrorReport(hEWDBC, pCursor,"PostGetAmpsForOriginByPickExec:ewdb_base_SQLFetchRows",1);
        return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
      }
    }
    ...
    for(; RowsDone < RowsRetrieved; RowsDone++)
    {
      ...
      /* CopyRowFromBuffertoUserBuffer */
      {
        ...
        /* initialize the current record in the caller's buffer */
        memset(&pBuffer[UCurr], 0, sizeof(EWDB_PeakAmpStruct));
        ...
        ******************************************************************/
        ...
        strncpy(pBuffer[UCurr].szExtSource, pTemp, sizeof(pBuffer[UCurr].szExtSource - 1));

        pTemp=(char *) 
           ((pSS->FieldArray[12].Ind*BCurr) +(int)(pSS->FieldArray[12].pVal));
        pTemp[pSS->FieldArray[12].pRetLens[BCurr]]=0;
        strncpy(pBuffer[UCurr].szExternalAmpID, pTemp, sizeof(pBuffer[UCurr].szExtSource - 1));
      }
  ...
  if(RowsRetrieved > BufferRecLen)
  {
    ...
    */
    ...
    if(ewdb_base_GetCursorRetCode(pCursor) != EWDB_SQL_ERROR_NO_DATA)
    {
      ewdb_base_ErrorReport(hEWDBC, pCursor,"PostGetGetAmpsForOriginByPickExec:ewdb_base_SQLFetchRows",2);
      return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
    }
    /* else */
...
}  /* End PostGetGetAmpsForOriginByPickExec() */
