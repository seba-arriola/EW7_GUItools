 ...
 *
 *    $Id: ewdb_internal_GetAmpsByEvent.c,v 1.5 2005/06/17 21:00:03 davidk Exp $
 *
 *    Revision history:
 *     $Log: ewdb_internal_GetAmpsByEvent.c,v $
...
static char SQL_STRING[] =
         ...
         "select idPeakAmp, idChan, dAmp1, tPeriod1, tAmp1, dAmp2, tPeriod2, tAmp2 "
         "from ALL_PEAKAMPS_FOR_EVENT "
...
static int PostGetAmpsByEventExec(EWDB_PeakAmpStruct * pBuffer, int BufferRecLen);
                                 ...
                                 int iBufferLen)
{
  ...
  {
    ewdb_base_ErrorReport(hEWDBC, pCursor,"ewdb_internal_GetAmpsByEvent(): ewdb_base_SQLExecute",1);
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  } 
  ...
  ****************************************************/
  ...
  {
    ewdb_base_ErrorReport (hEWDBC, hEWDBC,"ewdb_internal_GetAmpsByEvent(): ewdb_base_SQLCommit",2);
    return (ewdb_base_Disconnect (EWDB_RETURN_FAILURE));
  }
 
  if((*pNumAmpsFound = PostGetAmpsByEventExec(pAmpBuffer,iBufferLen)) 
     == EWDB_RETURN_FAILURE)
  {
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
                                       ...
                                       EWDB_OCIStatementStruct *pSS)
{
  ...
  if(!pLocalBuffer)
  {
    ...
    pSS->FieldArray[9].pVal=&Local_iMagType;
  ...
  if(!pLocalBuffer)
  {
    logit("","InitGetAmpsByEventStatement: malloc of pLocalBuffer "
          "failed! Returning.\n");
    return(EWDB_RETURN_FAILURE);
  }

  return(ewdb_base_RequestCursor(szStatement, pSS,0/*don't force rebind*/));
}  /* End InitGetAmpsByEventStatement() */
