 ...
 *    Revision history:                                     *
 *                                                          *
 *     $Log: ewdb_delete_waveforms_before_time.sql,v $
...
CREATE OR REPLACE FUNCTION Delete_Waveforms_Before_Time
(IN_tTime            number,
 IN_iMaxRecsToDelete number
) RETURN NUMBER

  /*  Delete_Waveforms_Before_Time Return codes:
           0 :   Success
...
as
  ...
  Cursor Waveform_cursor(Cursor_tTime number) is
    select * from WaveformDesc
...
begin
  ...
  for Curr in Waveform_cursor(IN_tTime) loop
    Temp_RetCode := Delete_Waveform(Curr.idWaveform,1/*bForce*/);
    if Temp < 0 then
      return(Temp);
    elsif Temp > 0 then
      Temp_RetCode := 1;
...
EXCEPTION
  WHEN OTHERS THEN
    insert into test values('Delete_Waveforms_Before_Time',IN_tTime,State);
	  return(-1);
END;
