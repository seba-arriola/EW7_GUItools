 ...
 *
 *    $Id: ewdb_api_GetCompTInfo.c,v 1.2 2005/06/10 16:27:59 davidk Exp $
 *    Revision history:
 *
 *    $Log: ewdb_api_GetCompTInfo.c,v $
...
/* ewdb_api_GetCompTInfo.c */

static char SQL_STRING[500];
static char SQL_STRING_BASE[] =
 "select idSite, 0/*idChan*/, idComp, sSta, sComp, sNet, sLoc, dAzm, dDip, "
         " dLat, dLon, dElev, tOn, tOff, idCompT, 0 /*idChanT*/ "
 "from ALL_COMPT_INFO";
...
/* Insertion Struct for GetCompTInfo statement */
...
/********************************
      FUNCTION PROTOTYPES
********************************/
static int PrepGetCompTInfoExec(EWDB_ChannelStruct * pCSCriteria,
                                EWDB_ChannelStruct * pCSMax, 
                                int iCriteria,
                                EWDB_Cursor * ppCursor);
static int PostGetCompTInfoExec(EWDB_ChannelStruct * pBuffer, 
                                int BufferRecLen);
static char * AddCriteria(char * MainString, int NumAdditions, 
                          char * pCriteria);
static int InitGetCompTInfoStatement(char * szStatement, 
...
/*********************************************************************** 
   Used to get the list of all Stations in the database, in a given time range,
	...
	pBuffer is a pointer to a buffer allocated by the caller and used
	 by GetCompTInfo to store the array of Stations.
  ...
  pCriteria is a pointer to the Stationstruct that contains the list formulation
   ...
   See the Stationstruct definition in ora_api.h for info on the other
	...
	pNumStations is the number of event structures returned by GetCompTInfo.
                                 ...
                                 int * pNumStationsRetrieved)
{
  ...
  if( PrepGetCompTInfoExec(pCSCriteria,pCSMaxCriteria,iCriteria,&pCursor)
      != EWDB_RETURN_SUCCESS )
  {
    logit("","ewdb_api_GetCompTInfo(): PrepGetCompTInfoExec() failed.\n");
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
  
  if( ewdb_base_SQLExecute(pCursor) )
  {
    ewdb_base_ErrorReport(hEWDBC, pCursor,"ewdb_api_GetCompTInfo(): ewdb_base_SQLExecute",1);
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  } 
  ...
  ****************************************************/
  ...
  {
    ewdb_base_ErrorReport(hEWDBC, hEWDBC,"ewdb_api_GetCompTInfo(): ewdb_base_SQLCommit",2);
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE) );
  }

  if((*pNumStationsFound=PostGetCompTInfoExec((EWDB_ChannelStruct *)pBuffer,iBufferLen)) 
      == EWDB_RETURN_FAILURE )
  {
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
...
} /* end ewdb_api_GetCompTInfo() */


static int InitGetCompTInfoStatement(char * szStatement, EWDB_OCIStatementStruct *pSS)
{
  ...
  if(!pLocalBuffer)
  {
    ...
    pSS->FieldArray[16].pVal=Local_sztStartTime;
    pSS->FieldArray[17].pVal=Local_sztEndTime;
    pSS->FieldArray[18].pVal=Local_szMinLat;
    pSS->FieldArray[19].pVal=Local_szMaxLat;
    pSS->FieldArray[20].pVal=Local_szMinLon;
    pSS->FieldArray[21].pVal=Local_szMaxLon;
    pSS->FieldArray[22].pVal=Local_szSta;
    pSS->FieldArray[23].pVal=Local_szComp;
    pSS->FieldArray[24].pVal=Local_szNet;
    pSS->FieldArray[25].pVal=Local_szLoc;
    pSS->FieldArray[26].pVal=&Local_idChan;

  }
  

  if(!pLocalBuffer)
    logit("","InitGetCompTInfoStatement: malloc of pLocalBuffer "
          "failed! Returning.\n");


  return(ewdb_base_RequestCursor(szStatement, pSS,0/*don't force rebind*/));
}  /* end InitGetCompTInfoStatement() */
    ...
    */
{
 ...
 if(InitGetCompTInfoStatement(SQL_STRING, &SSStatement) != EWDB_RETURN_SUCCESS)
  {
    *ppCursor = NULL;
    return(EWDB_RETURN_FAILURE);
  }
  ...
  *ppCursor=SSStatement.pCda;
  ...
  return(EWDB_RETURN_SUCCESS);
}  /* end PrepGetCompTInfoExec() */


static int PostGetCompTInfoExec(EWDB_ChannelStruct * pBuffer, int BufferRecLen)
{
    /* 
  ...
  while(!done)
  {
    ...
    if (ewdb_base_SQLFetchRows(pCursor, iRecordsPerBuffer))
    {
      ...
      {
        ewdb_base_ErrorReport(hEWDBC, pCursor,"PostGetCompTInfoExec:ewdb_base_SQLFetchRows",1);
        return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
      }
    }
    ...
    for(; RowsDone < RowsRetrieved; RowsDone++)
    {
      ...
      /* CopyRowFromBuffertoUserBuffer */
      {
        ...
        pBuffer[UCurr].tOffCompT = pBuffer[UCurr].tOff;
        pBuffer[UCurr].tOnCompT = pBuffer[UCurr].tOn;
  ...
  if (RowsRetrieved > BufferRecLen)
  {
    ...
    */
    ...
    if(ewdb_base_GetCursorRetCode(pCursor) != EWDB_SQL_ERROR_NO_DATA)
    {
      ewdb_base_ErrorReport(hEWDBC, pCursor,"PostGetCompTInfoExec:ewdb_base_SQLFetchRows",2);
      return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
    }
    /* else */
