 ...
 *
 *    $Id: ewdb_api_GetSelectedChannels.c,v 1.9 2005/06/27 15:30:01 davidk Exp $
 *    Revision history:
 *
 *    $Log: ewdb_api_GetSelectedChannels.c,v $
 *    Revision 1.9  2005/06/27 15:30:01  davidk
 *    Fixed bugs introduced during global fetch/replace during DB cleanup.
 *
 *    Revision 1.8  2005/06/10 16:27:59  davidk
 *    DB API Cleanup
 *
 *    Revision 1.7  2003/12/03 00:23:29  davidk
 ...
 *
 *    Revision 1.5  2003/08/21 00:40:58  davidk
 *    Restructured API code that deals with dynamic allocation of memory.
 *
 *    Revision 1.4  2003/03/28 18:20:27  davidk
 *    removed reintroduced bug and superfluous logging statement from v1.1.
 *
 *    Revision 1.3  2003/03/28 18:19:22  davidk
...
/* ewdb_api_GetSelectedChannels.c */

static char SQL_STRING[500];
static char SQL_STRING_BASE[] =
 "select idSite, idChan, idComp, sSta, sComp, sNet, sLoc, dAzm, dDip, "
         " dLat, dLon, dElev, tOn, tOff, dSampRate, dGain, idChanT, "
         " idCompT, tOnCompT, tOffCompT "
 "from ALL_CHANNEL_INFO_W_SITE";
...
static EWDB_OCI_SFS SQLParamsBindArray[] = 
{
  ...
  {0,1,20,0,0,OA_DOUBLE,"15dGain"},
  {0,1,20,0,0,OA_DOUBLE,"16dSampRate"},
  {0,1,0,0,0,OA_EWDBID, "17idChanT"},
  {0,1,0,0,0,OA_EWDBID, "18idCompT"},
  {0,1,20,0,0,OA_DOUBLE,"19tOnCompT"},
  {0,1,20,0,0,OA_DOUBLE,"20tOffCompT"},
  ...
  {0,1,0,0,0,OA_EWDBID, ":idchan"}
};

/* Insertion Struct for GetSelectedChannels statement */
...
/********************************
      FUNCTION PROTOTYPES
********************************/
static int PrepGetSelectedChannelsExec(EWDB_ChannelStruct * pCSCriteria,
                                EWDB_ChannelStruct * pCSMax, 
                                int iCriteria,
                                EWDB_Cursor * ppCursor);
static int PostGetSelectedChannelsExec(EWDB_ChannelStruct * pBuffer, 
                                int BufferRecLen);
static char * AddCriteria(char * MainString, int NumAdditions, 
                          char * pCriteria);
static int InitGetSelectedChannelsStatement(char * szStatement, 
...
/*********************************************************************** 
   Used to get the list of all Stations in the database, in a given time range,
	...
	pBuffer is a pointer to a buffer allocated by the caller and used
	 by GetSelectedChannels to store the array of Stations.
  ...
  pCriteria is a pointer to the Stationstruct that contains the list formulation
   ...
   See the Stationstruct definition in ora_api.h for info on the other
	...
	pNumStations is the number of event structures returned by GetSelectedChannels.
                                 ...
                                 int * pNumStationsRetrieved)
{
  ...
  {
    logit ("", "ewdb_api_GetSelectedChannels():Null pointer passed in!\n");
    return EWDB_RETURN_FAILURE;
  }

  if (iBufferLen <= 0)
  {
    logit ("", "ewdb_api_GetSelectedChannels():Bad iBufferLen passed in!\n");
    return EWDB_RETURN_FAILURE;
  }

  if( PrepGetSelectedChannelsExec(pCSCriteria,pCSMaxCriteria,iCriteria,&pCursor)
      != EWDB_RETURN_SUCCESS )
  {
    logit("","ewdb_api_GetSelectedChannels(): PrepGetSelectedChannelsExec() failed.\n");
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
  
  if( ewdb_base_SQLExecute(pCursor) )
  {
    ewdb_base_ErrorReport(hEWDBC, pCursor,"ewdb_api_GetSelectedChannels(): ewdb_base_SQLExecute",1);
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  } 
  ...
  ****************************************************/
  ...
  {
    ewdb_base_ErrorReport(hEWDBC, hEWDBC,"ewdb_api_GetSelectedChannels(): ewdb_base_SQLCommit",2);
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE) );
  }

  if((*pNumStationsFound=PostGetSelectedChannelsExec((EWDB_ChannelStruct *)pBuffer,iBufferLen)) 
      == EWDB_RETURN_FAILURE )
  {
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
...
}  /* end ewdb_api_GetSelectedChannels() */


static int InitGetSelectedChannelsStatement(char * szStatement, EWDB_OCIStatementStruct *pSS)
{
  ...
  if(!pLocalBuffer)
  {
    ...
    iRecordSize += pSS->FieldArray[14].Ind; /*dGain*/
    iRecordSize += pSS->FieldArray[15].Ind; /*dSampRate*/
    iRecordSize += sizeof(EWDBid); /*idChanT*/
    iRecordSize += sizeof(EWDBid); /*idCompT*/
    iRecordSize += pSS->FieldArray[18].Ind; /*tOnCompT*/
    iRecordSize += pSS->FieldArray[19].Ind; /*tOffCompT*/
    ...
    LastSize=pSS->FieldArray[14].Ind;  /* double dGain */
  
    pSS->FieldArray[15].pVal= (void *) (
      (int)(pSS->FieldArray[14].pVal)+(LastSize*iRecordsPerBuffer));
    LastSize=pSS->FieldArray[15].Ind;  /* double dSampRate */
    ...
    LastSize=pSS->FieldArray[18].Ind;  /* double tOnCompT */
  
    pSS->FieldArray[19].pVal= (void *) (
      (int)(pSS->FieldArray[18].pVal)+(LastSize*iRecordsPerBuffer));
    LastSize=pSS->FieldArray[19].Ind;  /* double tOffCompT */
  
  
    pSS->FieldArray[20].pVal=Local_sztStartTime;
    pSS->FieldArray[21].pVal=Local_sztEndTime;
    pSS->FieldArray[22].pVal=Local_szMinLat;
    pSS->FieldArray[23].pVal=Local_szMaxLat;
    pSS->FieldArray[24].pVal=Local_szMinLon;
    pSS->FieldArray[25].pVal=Local_szMaxLon;
    pSS->FieldArray[26].pVal=Local_szSta;
    pSS->FieldArray[27].pVal=Local_szComp;
    pSS->FieldArray[28].pVal=Local_szNet;
    pSS->FieldArray[29].pVal=Local_szLoc;
    pSS->FieldArray[30].pVal=&Local_idChan;
...
}  /* end InitGetSelectedChannelsStatement() */
    ...
    */
{
  ...
  /* Set the query to order by SNCL */
  strcat(SQL_STRING," ORDER BY sSta, sNet, sComp, sLoc");


  if(InitGetSelectedChannelsStatement(SQL_STRING, &SSStatement) != EWDB_RETURN_SUCCESS)
  {
    *ppCursor = NULL;
    return(EWDB_RETURN_FAILURE);
  }
  ...
  *ppCursor=SSStatement.pCda;
  ...
  return(EWDB_RETURN_SUCCESS);
}  /* end PrepGetSelectedChannelsExec() */


static int PostGetSelectedChannelsExec(EWDB_ChannelStruct * pBuffer, int BufferRecLen)
{
    /* 
  ...
  while(!done)
  {
    ...
    if (ewdb_base_SQLFetchRows(pCursor, iRecordsPerBuffer))
    {
      ...
      {
        ewdb_base_ErrorReport(hEWDBC, pCursor,"PostGetSelectedChannelsExec:ewdb_base_SQLFetchRows",1);
        return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
      }
    }
    ...
    for(; RowsDone < RowsRetrieved; RowsDone++)
    {
      ...
      /* CopyRowFromBuffertoUserBuffer */
      {
        ...
        /* handle gain and samplrate separately in case pChanProps is NULL */

        pBuffer[UCurr].idChanT=* (EWDBid *)((sizeof(int)*BCurr) + (int)(pSS->FieldArray[16].pVal));

        pBuffer[UCurr].idCompT=* (EWDBid *)((sizeof(int)*BCurr) + (int)(pSS->FieldArray[17].pVal));
        ...
        /* check for valid pBuffer[UCurr].pChanProps before stuffing data into
           that structure */
        if(pBuffer[UCurr].pChanProps)
        {
          pTemp=(char *) ((pSS->FieldArray[14].Ind*BCurr) + (int)(pSS->FieldArray[14].pVal) );
          pTemp[pSS->FieldArray[14].pRetLens[BCurr]]=0;
          pBuffer[UCurr].pChanProps->dSampRate=atof(pTemp);
          
          pTemp=(char *) ((pSS->FieldArray[15].Ind*BCurr) + (int)(pSS->FieldArray[15].pVal) );
          pTemp[pSS->FieldArray[15].pRetLens[BCurr]]=0;
          pBuffer[UCurr].pChanProps->dGain=atof(pTemp);
          
          pBuffer[UCurr].pChanProps->idChanT = pBuffer[UCurr].idChanT;
        }
        else
        {
          /* for now, let's be flexible and not complain about the null pointer */
        }
  ...
  if (RowsRetrieved > BufferRecLen)
  {
    ...
    */
    ...
    if(ewdb_base_GetCursorRetCode(pCursor) != EWDB_SQL_ERROR_NO_DATA)
    {
      ewdb_base_ErrorReport(hEWDBC, pCursor,"PostGetSelectedChannelsExec:ewdb_base_SQLFetchRows",2);
      return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
    }
    /* else */
...
}  /* end PostGetSelectedChannelsExec() */
