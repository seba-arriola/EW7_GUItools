 ...
 *
 *    $Id: ewdb_api_GetSelectedPickableStations.c,v 1.4 2005/06/10 16:27:59 davidk Exp $
 *    Revision history:
 *
 *    $Log: ewdb_api_GetSelectedPickableStations.c,v $
 *    Revision 1.4  2005/06/10 16:27:59  davidk
 *    DB API Cleanup
 *
 *    Revision 1.3  2005/01/12 17:04:00  davidk
 *    Increased buffer size for data retrieval
 *
 *    Revision 1.2  2005/01/12 16:52:17  davidk
 *    Rewrote SQL query to hopefully be much faster.
 *
 *    Revision 1.1  2004/11/19 16:58:49  davidk
 *    Function to retrieve just the stations from the database with which atleast one pick
 *    has been associated.
 *
 *
 * 
...
/* ewdb_api_GetSelectedPickableStations.c */

static char SQL_STRING[500];
static char SQL_STRING_BASE[] =
 "select a.idSite, a.idChan, a.idComp, a.sSta, a.sComp, a.sNet, a.sLoc,"
 "         a.dAzm, a.dDip, a.dLat, a.dLon, a.dElev,"
 "         a.tOn, a.tOff, a.idCompT, a.idChanT, a.tOnCompT, a.tOffCompT "
 " from ALL_STATION_INFO_W_SITE a "
 " where  a.idchan in (select idchan from pick) ";
...
static char SQL_STRING_OPT[][80] = 
    {
      ...
      "a.sComp = :comp_name",
      "a.sNet = :net_name",
      "(a.sLoc =  :loc_name or(a.sLoc IS NULL and :loc_name IS NULL))",
      "a.idChan = :idchan"
    };
...
/* Insertion Struct for GetSelectedPickableStations statement */
...
/********************************
      FUNCTION PROTOTYPES
********************************/
static int PrepGetSelectedPickableStationsExec(EWDB_ChannelStruct * pCSCriteria,
                                EWDB_ChannelStruct * pCSMax, 
                                int iCriteria,
                                EWDB_Cursor * ppCursor);
static int PostGetSelectedPickableStationsExec(EWDB_ChannelStruct * pBuffer, 
                                int BufferRecLen);
static char * AddCriteria(char * MainString, int NumAdditions, 
                          char * pCriteria);
static int InitGetSelectedPickableStationsStatement(char * szStatement, 
...
/*********************************************************************** 
   Used to get the list of all Stations in the database, in a given time range,
	...
	pBuffer is a pointer to a buffer allocated by the caller and used
	 by GetSelectedPickableStations to store the array of Stations.
  ...
  pCriteria is a pointer to the Stationstruct that contains the list formulation
   ...
   See the Stationstruct definition in ora_api.h for info on the other
	...
	pNumStations is the number of event structures returned by GetSelectedPickableStations.
                                 ...
                                 int * pNumStationsRetrieved)
{
  ...
  if( PrepGetSelectedPickableStationsExec(pCSCriteria,pCSMaxCriteria,iCriteria,&pCursor)
      != EWDB_RETURN_SUCCESS )
  {
    logit("","ewdb_api_GetSelectedPickableStations(): PrepGetSelectedPickableStationsExec() failed.\n");
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
  
  if( ewdb_base_SQLExecute(pCursor) )
  {
    ewdb_base_ErrorReport(hEWDBC, pCursor,"ewdb_api_GetSelectedPickableStations(): ewdb_base_SQLExecute",1);
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  } 
  ...
  ****************************************************/
  ...
  {
    ewdb_base_ErrorReport(hEWDBC, hEWDBC,"ewdb_api_GetSelectedPickableStations(): ewdb_base_SQLCommit",2);
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE) );
  }

  if((*pNumStationsFound=PostGetSelectedPickableStationsExec((EWDB_ChannelStruct *)pBuffer,iBufferLen)) 
      == EWDB_RETURN_FAILURE )
  {
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
...
}  /* end ewdb_api_GetSelectedPickableStations(): () */


static int InitGetSelectedPickableStationsStatement(char * szStatement, EWDB_OCIStatementStruct *pSS)
{
  ...
  if(!pLocalBuffer)
    logit("","InitGetSelectedPickableStationsStatement: malloc of pLocalBuffer "
          "failed! Returning.\n");

  return(ewdb_base_RequestCursor(szStatement, pSS,0/*don't force rebind*/));
}  /* end InitGetSelectedPickableStationsStatement() */
    ...
    */
{
  ...
  if(InitGetSelectedPickableStationsStatement(SQL_STRING, &SSStatement) != EWDB_RETURN_SUCCESS)
  {
    *ppCursor = NULL;
    return(EWDB_RETURN_FAILURE);
  }
  ...
  *ppCursor=SSStatement.pCda;
  ...
  return(EWDB_RETURN_SUCCESS);
}  /* end PrepGetSelectedPickableStationsExec() */


static int PostGetSelectedPickableStationsExec(EWDB_ChannelStruct * pBuffer, int BufferRecLen)
{
    /* 
  ...
  while(!done)
  {
    ...
    if (ewdb_base_SQLFetchRows(pCursor, iRecordsPerBuffer))
    {
      ...
      {
        ewdb_base_ErrorReport(hEWDBC, pCursor,"PostGetSelectedPickableStationsExec:ewdb_base_SQLFetchRows",1);
        return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
      }
    }
  ...
  if (RowsRetrieved > BufferRecLen)
  {
    ...
    */
    ...
    if(ewdb_base_GetCursorRetCode(pCursor) != EWDB_SQL_ERROR_NO_DATA)
    {
      ewdb_base_ErrorReport(hEWDBC, pCursor,"PostGetSelectedPickableStationsExec:ewdb_base_SQLFetchRows",2);
      return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
    }
    /* else */
...
}  /* end PostGetSelectedPickableStationsExec() */
