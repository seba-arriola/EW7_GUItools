/*
 *   THIS FILE IS UNDER RCS - DO NOT MODIFY UNLESS YOU HAVE
 *   CHECKED IT OUT USING THE COMMAND CHECKOUT.
 *
 *    $Id: ewdb_api_GetAlarmsRecipientList.c,v 1.6 2005/06/03 22:32:16 davidk Exp $
 *
 *    Revision history:
 *     $Log: ewdb_api_GetAlarmsRecipientList.c,v $
 *     Revision 1.6  2005/06/03 22:32:16  davidk
 *     DB API Cleanup
 *
 *     Revision 1.5  2005/03/03 17:50:46  mark
 *     Added bActive flag to recipient struct
 *
 *     Revision 1.4  2003/10/21 15:55:47  davidk
 *     Fixed a bug introduced in last "API Cleanup" change.
 *     Fixed code so that SSStatement params are correctly set in InitXXX().
 *
 *     Revision 1.3  2003/10/09 17:56:56  davidk
 *     Cleanup of API call, to match standard list-retrieval format.
 *
 *     Revision 1.2  2001/08/07 16:51:06  lucky
 *     Pre v6.0 checkin
 *
 *     Revision 1.1  2001/07/31 20:44:40  lucky
 *     Initial revision
 *
 *     Revision 1.3  2001/07/28 00:44:27  lucky
 *      State of the code after debugging and testing prior to v6.0 release freeze.
 *
 *     Revision 1.2  2001/05/15 18:47:41  davidk
 *     Moved functions around between the apps, DB API, and DB API INTERNAL
 *     levels.  Renamed functions and files.  Added support for amplitude
 *     magnitude types.  Reformatted makefiles.
 *
 *     Revision 1.1  2001/02/28 17:24:59  lucky
 *     Initial revision
 *
 *     Revision 1.1  2001/02/28 17:19:22  lucky
 *     Initial revision
 *
 */

#include <stdlib.h>
#include <stdio.h>
#include <ewdb_cli_base.h>
#include <ewdb_ew_oci_base.h>
#include <ewdb_ora_api.h>
#include <ewdb_oci_base_internal.h>

/*
    We work in two ways: (1) get all users, and
    (2) get a specific user
*/

static char SQL_STRING[256];

static char SQL_STRING_BASE[] =
        "select idRecipient, sDescription, dPriority, bActive "
        " from ALL_AlarmsRecipient_INFO ";

static char SQL_STRING_WHERE[] =
        "where idRecipient = :IN_idRecipient";


static EWDB_OCI_SFS SQLParamsBindArray[] =

{
  {0,1,0,0,0,OA_INT,   "1idRecipient"},
  {0,1,256,0,0,OA_SZ,  "2sDescription"},
  {0,1,0,0,0,OA_INT,   "3dPriority"},
  {0,1,0,0,0,OA_INT,   "4bActive"},
  {0,1,0,0,0,OA_INT,   ":IN_idRecipient"},
};

/* define the total number of fields to be bound */
#define NUM_FIELDS    5 


/* (SQL) Statement Struct for our statement */
static EWDB_OCIStatementStruct SSStatement;

/* Temporary Storage Buffer params */
#define BUFFERSIZE 8192
static int    iRecordSize;
static int    iRecordsPerBuffer;
static char * pLocalBuffer=NULL;


static  int     Local_idRecipient;

/********************************
      FUNCTION PROTOTYPES
********************************/
static int PrepGetAlarmsRecListExec (int, EWDB_Cursor *);
static int InitGetAlarmsRecListStatement (char *, EWDB_OCIStatementStruct *);
static int PostGetAlarmsRecListExec (EWDB_AlarmsRecipientStruct *, int);
/*******************************/


/* Returns ALL users in the AlarmsRecipient table */

int  ewdb_api_GetAlarmsRecipientList(EWDBid IN_idRecipient, 
                                    EWDB_AlarmsRecipientStruct *pRecipients, 
                                    int *pNumFound, int *pNumRetrieved, 
                                    int BufferLen)
{

  EWDB_Cursor pCursor;

    if ((pRecipients == NULL) || (pNumFound == NULL) || (pNumRetrieved == NULL) ||
                (BufferLen <= 0))
    {
        logit ("", "ewdb_api_GetAlarmsRecipientList(): Invalid arguments passed in.\n");
        return EWDB_RETURN_FAILURE;
    }

  ewdb_base_SetLastOraAPIActionTime ();

  if (ewdb_base_Reconnect () != EWDB_RETURN_SUCCESS)
  {
    logit ("", "ewdb_api_GetAlarmsRecipientList(): ewdb_base_Reconnect failed.\n");
    return (EWDB_RETURN_FAILURE);
  }

  if (PrepGetAlarmsRecListExec (IN_idRecipient, &pCursor) != EWDB_RETURN_SUCCESS)
  {
    logit ("", "ewdb_api_GetAlarmsRecipientList(): PrepGetAlarmsRecListExec failed.\n");
    return (ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }

  if (ewdb_base_SQLExecute (pCursor))
  {
    ewdb_base_ErrorReport (hEWDBC, pCursor,"ewdb_api_GetAlarmsRecipientList(): ewdb_base_SQLExecute", 1);
    return (ewdb_base_Disconnect (EWDB_RETURN_FAILURE));
  }

	/* Commit the transaction (all the previous inserts!)
	   In Case there is any auditing or logging or debug
	   changes made in the stored procedures.
	****************************************************/
	if (ewdb_base_SQLCommit (hEWDBC))
	{
		ewdb_base_ErrorReport (hEWDBC, hEWDBC,"ewdb_api_GetAlarmsRecipientList(): ewdb_base_SQLCommit",2);
		return (ewdb_base_Disconnect (EWDB_RETURN_FAILURE));
	}
	
  if ((*pNumFound = PostGetAlarmsRecListExec (pRecipients, BufferLen)) == EWDB_RETURN_FAILURE)
  {
    logit ("", "ewdb_api_GetAlarmsRecipientList(): PostGetAlarmsRecListExec failed.\n");
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }

  ewdb_base_SetLastOraAPIActionTime ();

  if (*pNumFound <= BufferLen)
  {
    *pNumRetrieved = *pNumFound;
    return (EWDB_RETURN_SUCCESS);
  }
  else
  {
    *pNumRetrieved = BufferLen;
    return (EWDB_RETURN_WARNING);
  }
}  /* end ewdb_api_GetAlarmsRecipientList() */


/******************* InitGetAlarmsRecListStatement *******************/
static int InitGetAlarmsRecListStatement(char *szStatement, 
                                         EWDB_OCIStatementStruct *pSS)
{
  int LastSize,i;  /* Size of last column array */

  if(!pLocalBuffer)
  {
    /*Allocate space for returned data - never freed */
    pLocalBuffer=malloc(BUFFERSIZE);

    iRecordSize=0;
    iRecordSize += sizeof(EWDBid); /*IN_idRecipient*/
    iRecordSize += pSS->FieldArray[1].Ind; /*sDescription*/
    iRecordSize += sizeof(int); /*dPriority*/
    iRecordSize += sizeof(int); /*bActive*/

    /* Make the number of records a multiple of 8 to avoid alignment problems on UNIX */
    iRecordsPerBuffer= (BUFFERSIZE/iRecordSize) & 0xfffffff8;
    
    /*Allocate space for row/col ret lens. - never freed */
    for(i=0;i<pSS->NumOfFields;i++)
    {
      pSS->FieldArray[i].pRetLens=malloc(iRecordsPerBuffer*EWDB_FIELD_RET_LEN);
    }

    pSS->FieldArray[0].pVal = &(pLocalBuffer[0]);
    LastSize = sizeof(int);  /* IN_idRecipient */
    
    pSS->FieldArray[1].pVal= (void *) ((int)(pSS->FieldArray[0].pVal) + 
      (LastSize * iRecordsPerBuffer));
    LastSize = pSS->FieldArray[1].Ind; /* sDescription */
    
    pSS->FieldArray[2].pVal= (void *) ((int)(pSS->FieldArray[1].pVal) + 
      (LastSize * iRecordsPerBuffer));
    LastSize = sizeof(int); /* dPriority */
    
    pSS->FieldArray[3].pVal= (void *) ((int)(pSS->FieldArray[2].pVal) + 
      (LastSize * iRecordsPerBuffer));
    LastSize = sizeof(int); /* bActive */
    
    pSS->FieldArray[4].pVal = &Local_idRecipient;
    
  } /* end if(!pLocalBuffer) */
  

  if(!pLocalBuffer)
  {
    logit("","InitGetAlarmsRecListStatement: malloc of pLocalBuffer "
          "failed! Returning.\n");
    return(EWDB_RETURN_FAILURE);
  }

  return(ewdb_base_RequestCursor(szStatement, pSS,0/*don't force rebind*/));
}  /* End InitGetAlarmsRecListStatement() */


/******************* PrepGetAlarmsRecListExec *******************/
static int PrepGetAlarmsRecListExec (int IN_idRecipient, EWDB_Cursor *ppCursor)
{

  if (ppCursor == NULL)
  {
    logit ("", "Invalid arguments passed in.\n");
    return EWDB_RETURN_FAILURE;
  }

  SSStatement.NumOfFields = NUM_FIELDS;
  SSStatement.FieldArray = SQLParamsBindArray;
  SSStatement.RecordSize = 0;

  if (IN_idRecipient < 0)
  {
    SQLParamsBindArray[4].UseField = FALSE;
    sprintf(SQL_STRING, "%s", SQL_STRING_BASE);
  }
  else
  {
    Local_idRecipient = IN_idRecipient;

    SQLParamsBindArray[4].UseField = TRUE;
    sprintf(SQL_STRING, "%s%s", SQL_STRING_BASE, SQL_STRING_WHERE);
  }

  *ppCursor = SSStatement.pCda;

  if(InitGetAlarmsRecListStatement(SQL_STRING, &SSStatement) != EWDB_RETURN_SUCCESS)
  {
    *ppCursor = NULL;
    return(EWDB_RETURN_FAILURE);
  }

  return (EWDB_RETURN_SUCCESS);
}


/******************* PostGetAlarmsRecListExec *******************/
static int PostGetAlarmsRecListExec(EWDB_AlarmsRecipientStruct *pBuffer, 
                                    int BufferRecLen)
{

  int           done = 0;
  int           RowsRetrieved = 0;
  int           RowsDone = 0;
  EWDB_Cursor   pCursor = SSStatement.pCda;
  char           *pTemp;
  int           BCurr,UCurr;
  int           RowsProcessed;
  EWDB_OCIStatementStruct  *pSS=&SSStatement;

  while (!done)
  {
    memset(pLocalBuffer, 0, BUFFERSIZE);
    
    if (ewdb_base_SQLFetchRows (pCursor, iRecordsPerBuffer))
    {
      if (ewdb_base_GetCursorRetCode(pCursor) == EWDB_SQL_ERROR_NO_DATA) 
        done=1;
      else
      {
        ewdb_base_ErrorReport (hEWDBC, pCursor,"PostGetAlarmsRecListExec:ewdb_base_SQLFetchRows", 1);
        return (ewdb_base_Disconnect (EWDB_RETURN_FAILURE));
      }
    }

    RowsRetrieved=ewdb_base_GetCursorRowsProcessedCount(pCursor);

    for(; RowsDone < RowsRetrieved; RowsDone++)
    {
      if(RowsDone >= BufferRecLen)
      {
        done=1;
        break;
      }

      /* Copy from DB rows into the user buffer */
      BCurr = RowsDone % iRecordsPerBuffer;
      UCurr = RowsDone;

      /* indx 0: IN_idRecipient */
      pBuffer[UCurr].idRecipient = *(int *)((sizeof (int) * BCurr) + 
                      (int)(pSS->FieldArray[0].pVal));

      /* indx 1: sDescription */
      pTemp = (char *) ((pSS->FieldArray[1].Ind * BCurr) + 
                    (int)(pSS->FieldArray[1].pVal));
      pTemp[pSS->FieldArray[1].pRetLens[BCurr]] = 0;
      strcpy (pBuffer[UCurr].sDescription, pTemp);

      /* indx 2: dPriority */
      pBuffer[UCurr].dPriority = *(int *)((sizeof (int) * BCurr) + 
                      (int)(pSS->FieldArray[2].pVal));

      /* indx 3: bActive */
      pBuffer[UCurr].bActive = *(int *)((sizeof (int) * BCurr) + 
                      (int)(pSS->FieldArray[3].pVal));

    } /* End for RowsDone < RowsRetrieved */
  }  /* End while !done */
  
  if(RowsRetrieved > BufferRecLen)
  {
    /* keep going till we get all of the rows,
       but ignore the contents, since we've
       filled up the user's buffer.
    */
    if(ewdb_base_GetCursorRetCode(pCursor) != EWDB_SQL_ERROR_NO_DATA)
    {
      while(!ewdb_base_SQLFetchRows(pCursor, iRecordsPerBuffer));
    }

    if(ewdb_base_GetCursorRetCode(pCursor) != EWDB_SQL_ERROR_NO_DATA)
    {
      ewdb_base_ErrorReport(hEWDBC, pCursor,
                       "PostGetAlarmsRecListExec:ewdb_base_SQLFetchRows",2);
      return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
    }
    /* else */
  }  /* End if(RowsRetrieved > BufferRecLen) */
  
  RowsProcessed = ewdb_base_GetCursorRowsProcessedCount(pCursor);

  ewdb_base_ReleaseCursor(pCursor);

  return(RowsProcessed);
}  /* End PostGetAlarmsRecListExec() */

