...
begin
	...
	in_addr := RTRIM (IN_sAddress);
	in_srvr := RTRIM (IN_sMailServer);
  ...
  /**************************************************/
	if IN_bisAudit = 0 then
		...
		State := 0;
		select idDelivery into Temp_idDelivery from EmailDelivery
			/** where sAddress = in_addr; ***/
			where idDelivery = IN_idDelivery;

		/* 
			found the existing delivery, so update it
		 */
		OUT_idDelivery := Temp_idDelivery;
	
            /* i found this existing delivery, so just update the existing one */
            update EmailDelivery
                    set EmailDelivery.sAddress = in_addr,
					    EmailDelivery.sMailServer = in_srvr
        ...
        select AuditEmailDeliverySeq.NEXTVAL into Temp_idDelivery from sys.dual;
        insert into AuditEmailDelivery
            (idDelivery, sAddress, sMailServer)
            values (Temp_idDelivery, IN_sAddress, IN_sMailServer);
...
EXCEPTION
	WHEN NO_DATA_FOUND THEN
		...
		if State = 0 then
			select EmailDeliverySeq.NEXTVAL into Temp_idDelivery from sys.dual;
