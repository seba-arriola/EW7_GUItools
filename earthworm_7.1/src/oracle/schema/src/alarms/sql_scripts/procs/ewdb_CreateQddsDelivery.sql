...
Temp_sDirectory           varchar(256);
Temp                      number;

begin

  Temp_sDirectory := RTRIM (IN_sQddsDirectory);
  ...
  if IN_bisAudit = 0 then
    ...
    State := 0;
    select idDelivery into Temp_idDelivery from QddsDelivery
      where sQddsDirectory = Temp_sDirectory; 
    ...
    OUT_idRecipientDelivery := Temp_idRecipientDelivery;
        ...
        select AuditQddsDeliverySeq.NEXTVAL into Temp_idDelivery from sys.dual;
        insert into AuditQddsDelivery
            (idDelivery, sQddsDirectory)
            values (Temp_idDelivery, IN_sQddsDirectory);
...
EXCEPTION
  WHEN NO_DATA_FOUND THEN
    ...
    if State = 0 then
      select QddsDeliverySeq.NEXTVAL into Temp_idDelivery from sys.dual;
      insert into QddsDelivery 
        (idDelivery, sQddsDirectory)
        values (Temp_idDelivery, IN_sQddsDirectory);
  ...
  WHEN OTHERS THEN
    Temp := SQLCODE;
    insert into test values('CreateQddsDel_EX ' || IN_idRecipient, Temp, State);
    OUT_idDelivery := -1;
    OUT_idRecipientDelivery := -1;
end;
