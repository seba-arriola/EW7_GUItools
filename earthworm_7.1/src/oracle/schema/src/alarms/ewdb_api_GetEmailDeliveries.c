 ...
 *
 *    $Id: ewdb_api_GetEmailDeliveries.c,v 1.7 2005/06/10 16:07:48 davidk Exp $
 *
 *    Revision history:
 *     $Log: ewdb_api_GetEmailDeliveries.c,v $
 *     Revision 1.7  2005/06/10 16:07:48  davidk
 *     DB Cleanup.  Fixed comments.
 *
 *     Revision 1.6  2005/06/03 22:32:16  davidk
 *     DB API Cleanup
 *
 *     Revision 1.5  2003/10/09 17:56:59  davidk
...
static char SQL_STRING_BASE[] =
        "select idDelivery, sAddress, sMailServer";

static char SQL_STRING_WHERE[] =
        " where idDelivery = :IN_idDelivery";

static char SQL_REAL_TBL[] = "from ALL_EmailDelivery_INFO";
static char SQL_AUDIT_TBL[] = "from ALL_AuditEmailDelivery_INFO";

static EWDB_OCI_SFS SQLParamsBindArray[] =

{
  {0,1,0,0,0,OA_INT,    "1idDelivery"},
  {0,1,256,0,0,OA_SZ,   "2sAddress"},
  {0,1,256,0,0,OA_SZ,   "3sMailServer"},
...
/********************************
      FUNCTION PROTOTYPES
********************************/
static int PrepGetEmailDelExec(int bIsAudit, int IN_idDelivery, EWDB_Cursor *ppCursor);
static int InitGetEmailDelStatement(char *, EWDB_OCIStatementStruct *);
static int PostGetEmailDelExec(EWDB_EmailDeliveryStruct *, int);
/*******************************/


int  ewdb_api_GetEmailDeliveries(int bIsAudit, int IN_idDelivery, 
                                ...
                                int BufferLen)
{
  ...
  if ((pDelivery == NULL) || (pNumFound == NULL) || (pNumRetrieved == NULL) ||
        (BufferLen <= 0))
  {
    logit ("", "ewdb_api_GetEmailDeliveries(): ERROR Invalid arguments passed in.\n");
  ...
  {
    logit ("", "ewdb_api_GetEmailDeliveries(): ERROR Call to ewdb_base_Reconnect failed.\n");
    return (EWDB_RETURN_FAILURE);
  }

  if (PrepGetEmailDelExec (bIsAudit, IN_idDelivery, &pCursor) != EWDB_RETURN_SUCCESS)
  {
    logit ("", "ewdb_api_GetEmailDeliveries(): PrepGetEmailDelExec failed.\n");
    return (ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }

  if (ewdb_base_SQLExecute (pCursor))
  {
    ewdb_base_ErrorReport (hEWDBC, pCursor,"ewdb_api_GetEmailDeliveries():ewdb_base_SQLExecute", 1);
    return (ewdb_base_Disconnect (EWDB_RETURN_FAILURE));
  }
  ...
  ****************************************************/
  ...
  {
    ewdb_base_ErrorReport (hEWDBC, hEWDBC,"ewdb_api_GetEmailDeliveries(): ewdb_base_SQLCommit",2);
    return (ewdb_base_Disconnect (EWDB_RETURN_FAILURE));
  }
  
  if ((*pNumFound = PostGetEmailDelExec (pDelivery, BufferLen)) == EWDB_RETURN_FAILURE)
  {
    logit ("", "ewdb_api_GetEmailDeliveries(): PostGetEmailDelExec failed.\n");
    return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
  }
...
}  /* end ewdb_api_GetEmailDeliveries() */


/******************* InitGetEmailDelStatement *******************/
static int InitGetEmailDelStatement(char *szStatement, EWDB_OCIStatementStruct *pSS)
{
  ...
  if(!pLocalBuffer)
  {
    ...
    iRecordSize += pSS->FieldArray[2].Ind; /*sMailServer*/
    ...
    pSS->FieldArray[2].pVal= (void *) ((int)(pSS->FieldArray[1].pVal) + 
      (LastSize * iRecordsPerBuffer));
    LastSize = pSS->FieldArray[2].Ind; /* sMailServer */
    
    pSS->FieldArray[3].pVal= &Local_idDelivery; /* incoming IN_idDelivery */
  } /* end if(!pLocalBuffer) */
  
  if(!pLocalBuffer)
  {
    logit("","InitGetEmailDelStatement(): malloc of pLocalBuffer "
          "failed! Returning.\n");
    return(EWDB_RETURN_FAILURE);
  }

  return(ewdb_base_RequestCursor(szStatement, pSS,0/*don't force rebind*/));
}  /* End InitGetEmailDelStatement() */
...
static int PrepGetEmailDelExec(int bIsAudit, int IN_idDelivery, EWDB_Cursor *ppCursor)
{
  ...
  if (ppCursor == NULL)
  {
    logit ("", "PrepGetEmailDelExec(): ERROR Invalid arguments passed in.\n");
    return EWDB_RETURN_FAILURE;
  }
  ...
  if (IN_idDelivery < 0)
  {
    ...
    rc = InitGetEmailDelStatement(SQL_STRING, &SSStatement);
  }
  else
  {
  ...
  *ppCursor = SSStatement.pCda;
  ...
  return(rc);
}  /* end PrepGetEmailDelExec() */


/******************* PostGetEmailDelExec *******************/
static int PostGetEmailDelExec (EWDB_EmailDeliveryStruct *pBuffer, int BufferRecLen)
{
  int           done = 0;
  int           RowsRetrieved = 0;
  int           RowsDone = 0;
  ...
  while (!done)
  {
    ...
    if (ewdb_base_SQLFetchRows (pCursor, iRecordsPerBuffer))
    {
      ...
      {
        ewdb_base_ErrorReport (hEWDBC, pCursor,"PostGetEmailDelExec:ewdb_base_SQLFetchRows", 1);
        return (ewdb_base_Disconnect (EWDB_RETURN_FAILURE));
      }
    }
    ...
    */
    ...
    if(ewdb_base_GetCursorRetCode(pCursor) != EWDB_SQL_ERROR_NO_DATA)
    {
      ewdb_base_ErrorReport(hEWDBC, pCursor,
                       "PostGetEmailDelExec:ewdb_base_SQLFetchRows",2);
      return( ewdb_base_Disconnect(EWDB_RETURN_FAILURE));
    }
    /* else */
