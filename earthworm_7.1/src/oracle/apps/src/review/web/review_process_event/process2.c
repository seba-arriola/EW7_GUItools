 ...
 *
 *    $Id: process2.c,v 1.1 2001/05/15 02:15:42 davidk Exp $
 *
 *    Revision history:
 *    $Log: process2.c,v $
...
int		html_delete (int);
int		html_insert (DBEventInfoStruct *, char *);
int		html_doalarms (DBEventInfoStruct *, int);
 ...
 ***************************************************************/
int		html_insert (DBEventInfoStruct *pEvent, char *author)
{
...
for (i = 0; i < pEvent->iNumChans; i++)
{
	...
	for (j = 0; j < pEvent->pChanInfo[i].iNumStaMags; j++)
	{
		...
		logit ("", "PhaseAmp %d: mag=%0.1f, wt=%0.1f, type=%s\n", j,
						pEvent->pChanInfo[i].PhaseAmps[j].dMag,
						pEvent->pChanInfo[i].PhaseAmps[j].dWeight,
						pEvent->pChanInfo[i].PhaseAmps[j].szMagType);
	}
	...
	if (PutDBEventInfo (pEvent, author, NULL, 0) != EWDB_RETURN_SUCCESS)
	{
...
***************************************************************/
int		html_doalarms (DBEventInfoStruct *pEvent, int Type)
{
