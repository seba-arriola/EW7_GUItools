...
/*#define			DEBUG	1*/
#define         N       1024		/* SD uses shorter buffers */
#define         NR      6		/* Number of windows */
#define         MEX     9		/* pow of 2 that yields N/2 */
...
/* Compute the average of the time series.  Note that data should never be in 
 ...
 mean should be on the order of 19-bits. */
 	 big=0;
 	 for(i = 0; i< N; i++) if(abs(ia[i]) > 4000000) big=1;
 	 
   iav = 0;
   if(big) for(i = 0; i < N; i++) iav += ia[i]/N;
   else {
