 ...
 *
 *    $Id: import_gen_pasv.c,v 1.10 2007/03/28 17:13:16 paulf Exp $
 *
 *    Revision history:
 *     $Log: import_gen_pasv.c,v $
...
static char ReceiverIpAdr[20];       /* Receiver's IP address             */
static int  ReceiverPort;            /* Receiver's well-known port number */

static char SenderIpAdr[20];         /* Sender's IP address for checking  */
...
int          PassiveSocket;           /* Passive Socket descriptor (receiver - import)   */
int          ActiveSocket;            /* Active Socket descriptor (sender - export)    	 */
...
int main( int argc, char **argv )
{
   ...
   char   startCharacter=STX;  /* frame start characer. (ASCII STX)  */
   char   endCharacter=ETX;    /* ditto frame end. (ASCII ETX)       */
   char   sender_ip[16];       /* Sender IP Address from inet_ntoa() */
   int    senderLen;
   int    on   = 1;
   int    quit = 0;
   int    sockErr;
 ...
 *************************************************************/
    ...
    ************************************************/ 
      ...
      if( strcmp(sender_ip, SenderIpAdr) != 0 ) 
      {
      ...
      {  /* we cried before, so now we have to let them know we're OK */
         sprintf( errText, "Connection accepted after %ld sec from IP address %s\n", 
                  time(&now)-acceptstart, sender_ip);
      ...
      if( state==ASSEMBLING_MESSAGE )
      {
          ...
          ******************/
            ...
            { 
               ...
               {
               /* Read from socket operation */
                  nr = recv_ew( (int)ActiveSocket, inBuffer, INBUFFERSIZE-1, 0, 
                                SocketTimeoutLength );
                  if( nr<=0 ) /* Connection Closed */
                  {
 ...
 */
    ...
    ************************************************/
      ...
      {
         if( WriteToSocket( (int)ActiveSocket, MyAliveString, &reclogo ) != 0 )
         {
