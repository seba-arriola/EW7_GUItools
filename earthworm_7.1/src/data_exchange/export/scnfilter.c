 ...
 *
 *    $Id: scnfilter.c,v 1.16 2005/11/16 23:58:29 dietz Exp $
 *
 *    Revision history:
 *     $Log: scnfilter.c,v $
 ...
 ************************************************/
   ...
   logit("o", "scnlfilter_init: configured to ship %d channels:", nSCNL );
   for( i=0; i<nSCNL; i++ ) {
 ...
 ********************************************************/
   ...
   {
   /* logit("e","scnlfilter: rejecting msgtype:%d - cannot read SCNL\n",
            intype ); */   /*DEBUG*/
      return( EW_PRIORITY_NONE );
   }
...
/* Gotta do it the linear way if wildcards were used! */
   ...
   if( match == NULL ) {
   /* logit("e","scnlfilter: rejecting msgtype:%d from %s %s %s %s\n",
