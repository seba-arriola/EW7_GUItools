...
/* EW error codesused by this Module
 ***********************************/
	...
	**********************************************/
	if(ImAModule) {	/* this is set in config_me() by the presence of module paramters in the config file*/
		if(HeartBeatInterval<=SLEEP_SEC){
			logit("e","%s Heartbeat Interval=%d. Shorter than minimum %d. Exiting\n", whoami, HeartBeatInterval, SLEEP_SEC);
			return(-1);
		}
  ...
  *************************/
  ...
  for ( isec=0; isec<=But->plt[i].mins*60; isec++) {
    ...
    if ((div(isec, major_incr)).rem == 0) 
    {
      ...
      sprintf(string, "%.2d", isec / 60);	/* label */
...
{
  char    string[200], datetxt[20], FileName[200]; 
  char    History[200], temp[175], whoami[NAMELEN];
  char    hour[10];
  FILE    *in, *out, *hist;
  int     i, j, nday;
  ...
  sprintf( FileName, "%swelcome.html", But->GifDir);   /* current copy of the full welcome.html file */
  out = fopen(FileName, "wb");
  if(out == 0L) 
    ...
    if(in == 0L) {
      ...
      while(fgets(string, 280, in)!=0L) fprintf(out, "%s", string);
      fclose(in);
    }
	...
	while(fgets(string, 128, hist)!=0L) {
	  ...
	  if(j>2 && string[0]!='#') {
	    ...
	    if ( nday < But->Days2Save ) 
	      fprintf(out, "<a href=\"%s.%s.gif\" > %s</a> | ", 
		      But->plt[i].SCNLnam, string, datetxt);
	    else
	      fprintf(out, "<a href=\"%s.%s.gif\" > %s</a>", 
		      But->plt[i].SCNLnam, string, datetxt);
	  }
	...
	else											/* @@@ start of else block "some problem occurred" @@@ */
	{
		...
		{											/* **** start of switch( io) statment */
			...
			/*******************/
				...
				sprintf(" %s server: %s Trace %s: No trace available. Wave server returned %c\n", 
...
void config_me( char* configfile, Butler *But )
{
  ...
  *****************************************************/
  ...
  But->NoMenuCount = 0;
  But->NoGIFCount  = 0;
  strcpy(But->IndexFile, "index.html");
