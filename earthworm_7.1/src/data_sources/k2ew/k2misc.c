 ...
 *
 *    $Id: k2misc.c,v 1.30 2007/02/26 17:16:53 paulf Exp $
 *
 *    Revision history:
 *     $Log: k2misc.c,v $
...
#define K2MI_MONMODE_MAX  6        /* number of times to try mon-mode cmd */
...
#define MAX_MSG_SIZE      256       /* Large enough for all faults in extended
                                       status message */
static char msg[MAX_MSG_SIZE];      /* Buffer for text messages */
...
int k2mi_init_blkmde(unsigned char *pseqnum)
{
  ...
  do     /* loop while attempting to activate and confirm K2 block mode */
  {
    ...
    {    /* loop while attempting to activate and confirm K2 monitor mode */
      ...
      if ( (slen = strlen(monmde_str_arr[idx])) == 1)
      {
...
int k2mi_ping_test(int count, unsigned char *pseqnum)
{
  ...
  {           /* send ping packet */
    slen = strlen(ping_test_data);       /* get string length */
    if (gcfg_debug > 3)
...
{
  int slen, index;
  unsigned long alarm, chanmask = 0x00000001;
  ...
  case PKC_MSG:            /* system message packet from K2 */
    ...
    {
       sprintf(msg, "K2 <%s>: pcdrv_raw_read error (bad flash memory)",
                g_stnid);
       k2mi_status_hb(g_error_ltype, K2STAT_BAD_DISK, msg);
    }
    if ( strncmp((char *)databuff, "pcdrv_raw_write, error", 22) == 0 )
    {
       sprintf(msg, "K2 <%s>: pcdrv_raw_write error (bad flash memory)",
                g_stnid);
       k2mi_status_hb(g_error_ltype, K2STAT_BAD_DISK, msg);
    }
    break;
  case PKC_SETALARM:       /* alarm message from K2 */
    ...
    {
      sprintf(msg, "K2 alarm received for channels: ");
      slen = strlen(msg);
      ...
      for (index = 0; index < ABS_MAX_CHANNELS; index++)
      {
        if (alarm & chanmask)
        {
          sprintf(&msg[slen], "%d ", index + 1);
          slen = strlen(msg);
        }
        chanmask = chanmask << 1L;
      }
      logit("et","%s\n", msg);
    }
    else
...
void k2mi_report_status( struct STATUS_INFO *pk2stat)
{
  ...
  sprintf(msg, "K2 Time = %s", asctime(gmtime(&timevar)));
  msg[strlen(msg)-1] = '\0';         /* delete the newline left by ctime() */

  switch (pk2stat->clockSource)
  {
  case 0:
    strcat(msg, " set from RTC");
    break;
  case 1:
    strcat(msg, " set from Keyboard");
    break;
  case 2:
    strcat(msg, " set from External Pulse");
    break;
  case 3:
    strcat(msg, " set from Internal GPS");
    break;
  default:
    strcat(msg, " set from (?)");
  }
  ...
  if (tempbat > 0) /* K2 says battery voltage is zero when it is charging */
  {
    ...
    {
    sprintf(msg, "K2 <%s>: lost external power; battery voltage: %0.1f V", 
		g_stnid, batv);
    k2mi_status_hb(g_error_ltype, K2STAT_ON_BATT, msg);
    rep_on_batt = 1;
    }
  }
  else
  {
    ...
    {
    sprintf(msg, "K2 <%s>: external power restored", g_stnid);
    k2mi_status_hb(g_error_ltype, K2STAT_OFF_BATT, msg);
    rep_on_batt = 0;
    rep_low_batt = 0;
    }
  }
  ...
  {
    sprintf(msg, "K2 <%s>: low battery voltage: %0.1f V", 
		g_stnid, batv);
    k2mi_status_hb(g_error_ltype, K2STAT_LOW_BATT, msg);
    rep_low_batt = 1;
  }
  ...
  {
    sprintf(msg, "K2 <%s> hardware fault", g_stnid);
    k2mi_status_hb(g_error_ltype, K2STAT_HW_FAULT, msg);
    rep_hwstat = 1;
  }
  else if (gcfg_hwstat_alarm && rep_hwstat == 1)
  {
    sprintf(msg, "K2 <%s> hardware fault cleared", g_stnid);
    k2mi_status_hb(g_error_ltype, K2STAT_HWFLT_CLR, msg);
    rep_hwstat = 0;
  }
  ...
  {
    sprintf(&msg[strlen(msg)], " %c: ", 'A' + index);
    ...
    else
    {
      ...
      sprintf(&msg[strlen(msg)], "%d %cB FREE", wtemp2, tempchar);
...
void k2mi_report_extstatus( struct EXT2_STATUS_INFO *pk2stat, int ext_size)
{
  ...
  /* Last restart time */
  memcpy(&ul_dummy, ((char *)pk2stat) + 2, sizeof(long));
  ...
  if (fault != (unsigned short)0)
  {
    ...
    if (fault & FAULT_RAM)
      sprintf(&msg[strlen(msg)], "ram error at %lX; ",
              BYTESWAP_UINT32(pk2stat->lastRAMError));
...
/* System temperature
  ...
  {
    sprintf(msg, "K2 <%s>: low temperature: %d.%1d C", 
		g_stnid, temp / 10, temp % 10);
    k2mi_status_hb(g_error_ltype, K2STAT_LOW_TEMP, msg);
    rep_temp = 1;
  }
  else if ((int)temp > gcfg_temphi_alarm && rep_temp == 0)
  {
    sprintf(msg, "K2 <%s>: high temperature: %d.%1d C", 
		g_stnid, temp / 10, temp % 10);
    k2mi_status_hb(g_error_ltype, K2STAT_HIGH_TEMP, msg);
    rep_temp = 1;
  }
  else if (rep_temp == 1)
  {
    sprintf(msg, "K2 <%s>: temperature OK: %d.%1d C", 
		g_stnid, temp / 10, temp % 10);
    k2mi_status_hb(g_error_ltype, K2STAT_OK_TEMP, msg);
    rep_temp = 0;
  }
   ...
   ***************************************************************/
   {
      ...
      if ( (gcfg_gpslock_alarm >= 0.0) && (now > year2000) && (timevar1 > year2000) )
      {
         ...
         if ( timeSinceLastLock >= gcfg_gpslock_alarm )
         {
            ...
            if ( prev_gpslock == 1 )
            {
               sprintf(msg, "K2 <%s>: No GPS lock for %.1lf hours.", 
			g_stnid, timeSinceLastLock );
               k2mi_status_hb(g_error_ltype, K2STAT_GPSLOCK, msg);
            }
            prev_gpslock = 0;
         }
         else
         {
            if ( prev_gpslock == 0 ) 
	    {
               sprintf(msg, "K2 <%s>: GPS lock acquired.", g_stnid);
               k2mi_status_hb(g_error_ltype, K2STAT_GPSLOCK, msg);
	    }
            prev_gpslock = 1;
         }
