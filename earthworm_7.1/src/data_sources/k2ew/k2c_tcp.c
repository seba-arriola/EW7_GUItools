 ...
 *
 *    $Id: k2c_tcp.c,v 1.9 2003/08/22 20:12:13 friberg Exp $
 *
 *    Revision history:
 *     $Log: k2c_tcp.c,v $
...
int k2c_init_io(s_gen_io *gen_io, int toutms)
{
  ...
  if (sockInit == 0)
  {
    ...
    saddr.sin_port = htons((unsigned short)gen_io->tcp_io.k2_port);
     ...
     */
    if ( (addr = inet_addr(gen_io->tcp_io.k2_address)) != INADDR_NONE )
    {
      saddr.sin_addr.s_addr = addr;
    }
    else
    {       /* it's not a dotted quad IP address */
      if ( (hp = gethostbyname(gen_io->tcp_io.k2_address)) == NULL)
      {
        logit("et", "k2c_init_io: invalid K2 address <%s>\n",
              gen_io->tcp_io.k2_address);
        return K2R_ERROR;
      }
