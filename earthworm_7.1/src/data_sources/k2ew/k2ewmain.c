 ...
 *
 *    $Id: k2ewmain.c,v 1.37 2005/09/15 13:56:19 friberg Exp $
 *
 *    Revision history:
 *     $Log: k2ewmain.c,v $
...
#define K2EW_VERSION_STR "2.41" /* version string for program */
...
#define K2M_PCKT_RELIDX(stm,dseq) \
   (((long)(dseq)-(long)cur_dataseq)*g_numstms+(unsigned char)(stm)-cur_stmnum)
...
unsigned long g_pktout_okcount=0L;/* # of packets sent to Earthworm OK */
unsigned long g_seq_errcnt=0L;    /* number of output sequence errors */
unsigned long g_skip_errcnt=0L;   /* number of packets skipped */
unsigned long g_trk_dataseq=0L;   /* output thread tracked data seqence */
...
long gcfg_ring_key=0L;                 /* key to ring buffer k2ew dumps data */
...
int main(int argc,char **argv)
{
  ...
  static unsigned long cur_dataseq,dataseq,rr_dataseq,msk,strmmsk,
                       acqmsk, rstrt_dataseq =0, last_sec = 0;
  static long idx,c;
  static unsigned long pktin_okcount,missing_errcnt,unexpect_errcnt,
  ...
  static long databuff[K2PM_MIN_DBUFSIZ+2];
  ...
  if (g_validrestart_flag == 0)
  {
    ...
    msk = (unsigned long)1;           /* initialize bit mask position */
    for(idnum=0; (unsigned char)idnum<K2_MAX_STREAMS; ++idnum)
    {      /* for each possible stream */
      if((msk & strmmsk) != (unsigned long)0)
    ...
    g_trk_dataseq = cur_dataseq = (unsigned long)0;
  }
  else
...
#ifdef SIGBREAK
  signal(SIGBREAK,k2ew_signal_hdlr);   /* keyboard break */
#endif
      ...
      {     /* stream and data sequence numbers do not match "current" */
        ...
        {   /* packet position is "ahead" of current packet expected */
          ...
          else   /* packet position is too far "ahead" of current packet; */
          {
            ...
            k2cb_check_waits((unsigned long)-1); /* Clear all wait pkts */
          }
          ...
          {      /* "waiting" block not filled */
            ...
            {    /* no matching "waiting" block was found */
              ...
              if (datahdr.Seconds > last_sec)
              {
                ...
                k2cb_check_waits((unsigned long)-1);  /* Clear wait pkts */
                   /* put "current" packet position in sync with received */
                cur_stmnum = datahdr.StreamNum;    /* take new stream number */
                cur_dataseq = datahdr.DataSeq;     /* take new data sequence # */
                   /* enter received data block into circular buffer */
    ...
    {    /* error retrieving SDS block or number of data entries mismatches */
      ...
      {       /* returned code is not one the "fatal" errors */
        ...
        {     /* SDS packet was received and its header interpreted OK   */
          ...
          logit("et","Error (count=%lu) on received packet (stm#=%hu, "
                     "dseq=%lu)\n", packet_errcnt,
                       (unsigned short)(datahdr.StreamNum),datahdr.DataSeq);
          if(rerequest_idnum == K2CB_DBFIDX_NONE &&
  ...
  {      /* loop termination was caused by "signal" event */
    sprintf (msg_txt, "Signal event (%d) detected. Exiting program: %s", k2ew_signal_val,
			 argv[1]);
