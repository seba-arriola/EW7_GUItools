 ...
 *
 *    $Id: nmxp_packet.h,v 1.11 2006/01/04 17:54:33 dietz Exp $
 *
 *    Revision history:
 *     $Log: nmxp_packet.h,v $
...
{
  long  firstsample;   /* 4 byte  first sample value               */
...
{
  int  ndata;
  long data[NMXP_MAX_SAMPLE_PER_BUNDLE];
} NMXPDATAVAL;
...
typedef struct _NMXPRCVSLOTERRBUNDLE 
{
  ...
  long  sec;           /* 4 byte  whole seconds since 1970/01/01   */
  long IPaddr;         /* 4 byte  IP Address                       */
  long badPkt;         /* 4 byte  no. of bad pkts since start of TDMA */
  long goodPkt;        /* 4 byte  no. of good pkts since start of TDMA */
...
typedef struct _NMXPBURSTBUNDLE 
{
  ...
  long goodBurst;      /* 3 byte  no. of good burst since start of TDMA */
  long badBurst;       /* 3 byte  no. of bad burst since start of TDMA */
...
typedef struct _NMXPLIBRASYSTIMEQUAL 
{
  ...
  long  sec;           /* 4 byte  whole seconds since 1970/01/01   */
  long  tquality;      /* 4 byte  system time quality              */
...
typedef struct _NMXPTIMESERVERTIMEPLL
{
  ...
  long  sec;           /* 4 byte  whole seconds since 1970/01/01  */
  long  subsec;	       /* 3 byte  subsec time in fast counts      */
  char  status;        /* 1 byte  Current state, many bit modes   */
  long  timeError;     /* 4 byte  time error in fast counts       */
  char  freqError;     /* 1 byte  measured freq error (0.1ppm) * 0.96 to get Hz */
  long  timeSince;     /* 3 byte  time since GPS lock loss        */
