 ...
 *
 *    $Id: nmxsrv_socket.c,v 1.1 2002/07/09 18:13:38 dietz Exp $
 *
 *    Revision history:
 *     $Log: nmxsrv_socket.c,v $
 ...
 **********************************/
   ...
   server.sin_addr.S_un.S_addr = inet_addr( ipadr );

   if ((int)server.sin_addr.S_un.S_addr == -1)
   {
      logit( "et", "naqs2ew: inet_addr failed for address <%s>\n",
             ipadr );
      return NMX_FAILURE;
   }
