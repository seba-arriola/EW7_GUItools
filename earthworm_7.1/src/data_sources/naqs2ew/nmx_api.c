 ...
 *
 *    $Id: nmx_api.c,v 1.8 2003/10/07 17:59:53 dietz Exp $
 *
 *    Revision history:
 *     $Log: nmx_api.c,v $
 ...
 ****************************************/
   phd = msg;
   tmp = (int) htonl( (unsigned long)nhd->signature );
   memcpy( phd, &tmp, sizeof(int) );
   phd += sizeof( int );

   tmp =  (int) htonl( (unsigned long)nhd->msgtype );
   memcpy( phd, &tmp, sizeof(int) );
   phd += sizeof( int );

   tmp =  (int) htonl( (unsigned long)nhd->msglen );
   memcpy( phd, &tmp, sizeof(int) );
 ...
 *********************************************************/
   ...
   crc32_update( (unsigned char *)conreq->user, strlen(conreq->user) );

   tmp = htonl( (unsigned long) conreq->DAPversion );
   memcpy( pbuf, &tmp, sizeof(long) );        pbuf += sizeof(long);
   crc32_update( (unsigned char *)&tmp, sizeof(long) );

   tmp = (int) htonl( (unsigned long)conreq->tconnect );
   memcpy( pbuf, &tmp, sizeof(long) );        pbuf += sizeof(long);
   crc32_update( (unsigned char *)&tmp, sizeof(long) );

/* Add the password to the CRC, but not to the msg.
   Then put the CRC in the msg in network byte order */
   crc32_update( (unsigned char *)conreq->pswd, strlen(conreq->pswd) );

   tmp = (int) htonl( crc32_value() );
   memcpy( pbuf, &tmp, sizeof(long) ); 
 ...
 ********************************************/
   msglen = sizeof(int);
   if( term->note != NULL ) msglen += strlen(term->note);
   if( nmx_checklen( buf, buflen, NMXHDR_LEN+msglen ) != 0 ) {
      return( NMX_FAILURE );
   }
...
/* Add the message contents, in network byte order
 *************************************************/
   tmp = (int) htonl( (unsigned long)term->reason );
...
/* Add the message contents, in network byte order
 *************************************************/
   tmp = (int) htonl( (unsigned long)nkeys );
   memcpy( pbuf, &tmp, sizeof(int) ); pbuf += sizeof( int );

   for( ik=0; ik<nkeys; ik++ )
   {
      tmp = (int) htonl( (unsigned long)keys[ik] );
      memcpy( pbuf, &tmp, sizeof(int) ); pbuf += sizeof( int );
   }
   tmp = (int) htonl( (unsigned long)delay );
   memcpy( pbuf, &tmp, sizeof(int) ); pbuf += sizeof( int );

   tmp = (int) htonl( (unsigned long)format );
   memcpy( pbuf, &tmp, sizeof(int) ); pbuf += sizeof( int );

   tmp = (int) htonl( (unsigned long)sendbuf );
...
/* Add the message contents, in network byte order
 *************************************************/
   tmp = (int) htonl( (unsigned long)instrid );
   memcpy( pbuf, &tmp, sizeof(int) ); pbuf += sizeof( int );

   tmp = (int) htonl( (unsigned long)dtype );
   memcpy( pbuf, &tmp, sizeof(int) ); pbuf += sizeof( int );

   tmp = (int) htonl( (unsigned long)chan );
   memcpy( pbuf, &tmp, sizeof(int) );
 ...
 ********************/
 ...
 /*logit("et","inst:%d ch:%d seqnum:%d\n",hdr.instrumentid,hdr.chan,hdr.seqnum );*/ /*DEBUG*/
   ...
   if(DEBUG) {
     ...
     logit("e","\ntstart:  %s (%.4lf)\n", stime, tstartval );
     logit("e","header startval (3byte):  %d\n", startval );
   }
 ...
 *****************************************/
   ...
   {
      int ndata = 0;  /* number of data samples unpacked from bundle */
   ...
   /* For Trident data, an extended header bundle follows the header */
      if( ibundle==1 && pbundle[0]==NMXP_EXTENDEDHDR_BUNDLE ) {  
        ...
        if(DEBUG) {
          logit("e","exthdr startval (4byte):  %d\n", xhdr.firstsample );
          logit("e","exthdr calibstatus bit0:%d  bit1:%d  bit2:%d\n",
   ...
   /* OK, it must be a real compressed data bundle */
      ...
      if(DEBUG) {
        printf("bundle%2d: ",ibundle); 
        for( i=0; i<xdat.ndata; i++ ) printf(" %d", xdat.data[i] );
        printf("\n"); 
      }
    ...
    ************************************/
      memcpy( pdata, xdat.data, xdat.ndata*sizeof(long) );
      pdata += xdat.ndata;
   }
 ...
 ****************************************************************/
   ...
   if(DEBUG) {
     ...
     logit("e","tend:  %s (%.4lf)\nendval:  %d\n", 
            stime, tend, naqsbuf->data[naqsbuf->nsamp-1] );
   }
