...
The run_16chan directory is to help configure SrParxchEwSend so it can
...
files appropriate for a basic version of this configuration.
...
SrParxchEwSend module acquires data and the wave_serverV module saves
...
The following steps describe how to run EW7.0 with 2 PAR8CH + 1 PARGPS.
Linux users should replace references to c:\earthworm with
/usr/local/earthworm:

1. Install the official version of Earthworm v7.0
   A. Download the "Windows Binary (zip)" from
      folkworm.ceri.memphis.edu/ew-dist/index.html
      the file is named v7.0-win-bin.zip.  Linux users should get
      the Linux tar file instead.
   B. Create the directory c:\earthworm and unzip v7.0 into this
      directory.
...
5. Update srparxchewsend in the earthworm directory from step 1 to the latest
   as follows:
   A. Go to the c:\earthworm\v7.0\src\data_sources\srparxchewsend directory
   B. Delete the contents of this directory
   C. Replace them with the files in \sr\parxch\extras\earthworm
   D. Copy run_v7.0\srparxchewsend.exe to c:\earthworm\v7.0\bin\.

6. Set up your run_symres directory
   A. Go to c:\earthworm\v7.0 and create the run_symres directory
   B. Go to c:\earthworm\v7.0\run_symres and create directories data, logs,
      and params
   C. Fill the params directory with the files in run_16chan\params
   D. Review the srparxchewsend.d file and edit the AtodModelName,
...
7. Start earthworm
   A. Open a command prompt or xterm and go to c:\earthworm\v7.0\run_symres\params
   ...
   C. Type startstop.  This should start 
      ...
      srparxchewsend to acquire data
      wave_serverV to store the acquired data

8. View the data
   A. Open another command prompt and go to c:\earthworm\v7.0\run_symres\params 
...
9. Check the logs to see if there are any problems, 
   A. Review the contents of the log files in C:\earthworm\v7.0\run_symres\log
...
10. Once you have verified that EW and PARxCH work together, you can
   ...
   The URL path for reading existing newsgroup questions and answers is:

     lyris.nmt.edu/read/?forum=earthw

   If you want to post your own question, you will need to subscribe at:

     lyris.nmt.edu/read/all_forums/subscribe?name=earthw
