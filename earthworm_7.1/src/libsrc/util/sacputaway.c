 ...
 *
 *    $Id: sacputaway.c,v 1.27 2005/06/13 15:00:46 patton Exp $
 *
 *    Revision history:
 *     $Log: sacputaway.c,v $
 ...
 */
{
  ...
  if (rc == -2)
  {
    ...
    logit("e", "%d %d %s (%.4f) %s (%.4f) %.1f %s.%s.%s.%s %s %s %s %s\n",
          ...
          wf->loc, wf->version, wf->datatype, wf->quality, wf->pad);
  ...
  if (datatype == 'n')
  {
    logit("et", "SACPABase_write_trace: ffunsupported datatype: %s\n", datatype);
    return( EW_FAILURE );
  }
  ...
  while (1) 
  {
    ...
    if ((long) msg_p >= ((long) pTrace->actLen + (long) pTrace->pBuf)) 
    {
      if (SACPAB_Debug == 1)
    ...
    if ( endtime + ( 1.0/samprate ) * dGapThresh < starttime ) 
    {
      ...
      if (SACPAB_Debug == 1)
      	logit("e", "gap in %s.%s.%s.%s: %lf: %lf, fill with %d samples\n", 
	      SAC_szSta, SAC_szChan, SAC_szNet, SAC_szLoc,
  ...
  sacheadp->npts = (long) nsamp_this_scnl;  /* samples in trace */
  ...
  time = gmtime( &ltime );
  sacheadp->nzyear = (long)time->tm_year + 1900l; /* calendar year of reference time */
  sacheadp->nzjday = (long)time->tm_yday + (long)1;  /* julian day, 0 - 365 */
  sacheadp->nzhour = (long)time->tm_hour;
  sacheadp->nzmin  = (long)time->tm_min;
  sacheadp->nzsec  = (long)time->tm_sec;
  sacheadp->nzmsec = (long)((SAC_starttime - (long)SAC_starttime) * 1000.0);
...
int SACPABase_write_parametric(SAC_ArrivalStruct * pArrival, int WaveType)
{
  ...
  if (WaveType == PWAVE)
  {
    ...
    sacheadp->ka[2]= pArrival->iPhaseWt + '0';
  ...
  else if (WaveType == SWAVE)
  {
    ...
    sacheadp->kt0[2]= pArrival->iPhaseWt + '0';
...
int SACPABase_end_scnl (void)
{
  ...
  long   i, npts;           /* number of SAC data points */
  float *sac_p;
...
int SAC_SwapLong(long * pValue, char cDataType)
{
  ...
  return(EW_SUCCESS);
#else
  logit("e","SAC_SwapLong():Error! Unable to determine platform! "
