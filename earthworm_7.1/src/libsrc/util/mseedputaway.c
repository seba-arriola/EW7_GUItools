...
static void MSEEDPA_write_record (char *record, int reclen);
...
int MSEEDPA_next_ev (char *output_dir,char *e_date, char *e_time, int debug)
{
  ...
  /* Open the file */
  if ( (fp = fopen(mseed_filename,"w+")) == NULL)
  {
...
int MSEEDPA_next (TRACE_REQ *trace_req, double gap_thresh, int debug)
{
  ...
  long           *l_data;                      /* temporary pointer for casting data */
  ...
  int            psamples;                     /* output from pack routine - number of samples packed */
  ...
  if (debug)
    logit("e","MSEEDPA_next: total trace length = %d\n",trace_req->actLen);
  ...
  { 
    ...
    {
      logit("et","MSEEDPA_next: changing samplerate (%d) for <%s.%s.%s.%s>\n",
                 trace_head->samprate, trace_head->sta, trace_head->chan, 
	         trace_head->net, trace_head->loc);
      return( EW_FAILURE );
    }
    ...
    if (first_trace)
    {
      first_samp = (long)(trace_head->samprate * (trace_req->reqStarttime - trace_head->starttime));
      mst->numsamples += trace_head->nsamp - first_samp;
      mst->starttime = MS_EPOCH2HPTIME(trace_req->reqStarttime);
      first_trace = 0;
    }
    else
    {
      ...
      mst->starttime = MS_EPOCH2HPTIME(trace_head->starttime);
    }

    /* Probably only want part of final trace too - end check based on length of request buffer */
    if ((long)(msg_p + sample_size * trace_head->nsamp) >= ((long)trace_req->actLen + (long)trace_req->pBuf))
    {
      final_samp = (long) (trace_head->samprate * (trace_req->reqEndtime - trace_head->starttime) + 1);
    ...
    case 'i':
      ...
      for (i=first_samp, j=0; i<final_samp; i++, j++)
        ((long*)mst->datasamples)[j] = l_data[i];
      break;
    ...
    precords = mst_pack (mst, &MSEEDPA_write_record, 4096, STEIM2, byte_order_flag, &psamples, 1, debug, NULL);
    if (precords == 0)
    {
    ...
    if (debug)
      logit("e","MSEEDPA_next: Packed %d samples into %d records\n", psamples, precords);
