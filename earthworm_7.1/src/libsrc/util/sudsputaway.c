 ...
 *
 *    $Id: sudsputaway.c,v 1.11 2004/07/16 20:44:00 lombard Exp $
 *
 *    Revision history:
 *     $Log: sudsputaway.c,v $
 *     Revision 1.11  2004/07/16 20:44:00  lombard
 ...
 *
 *     Revision 1.10  2003/10/03 20:10:36  cjbryan
 *     corrected initialization of SUDS structures
 *
...
void init_struct_SUDS_DESCRIPTRACE(SUDS_DESCRIPTRACE *dt)
{
  ...
  dt->dt_name.inst_type = atoi(NOLIST);

  dt->localtime = atoi(NODATA);
  dt->datatype = NOCHAR;
  dt->descriptor = NOCHAR;
  dt->digi_by = atoi(NOLIST);
  dt->processed = atoi(NOLIST);
