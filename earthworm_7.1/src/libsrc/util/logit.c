 ...
 *
 *    $Id: logit.c,v 1.13 2006/04/05 19:34:46 stefan Exp $
 *
 *    Revision history:
 *     $Log: logit.c,v $
...
int get_prog_name2(char *szFullName, char * szProgName, int iProgNameBufferLen)
{
	...
	{
		fprintf(stderr, "get_prog_name2() Null Pointer passed in. szFullName(%u), szProgName(%u).\n",
            szFullName,szProgName);
		return EW_FAILURE;
	}
...
int get_prog_name (char *full_name, char *prog_name)
{
	...
	{
		fprintf(stderr, "get_prog_name() Null Pointer passed in. full_name(%u), prog_name(%u).\n",
            full_name, prog_name);
		return EW_FAILURE;
	}
