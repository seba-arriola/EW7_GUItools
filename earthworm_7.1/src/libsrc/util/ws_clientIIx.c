...
#define wsMAX_REPLY_LEN  ((wsMAX_SCNLs_PER_MENU * 80/*chars per line*/) + 80 /*initial response*/)
/* globals */
                            ...
                            WS_MENU_REC * pMenu)
{
  ...
  pTemp = (WS_PSCNL) malloc((pEnv->iNumPSCNLs + SCNLsFound)  
                            * sizeof(WS_PSCNL_REC));
  if(WS_CL_DEBUG & WS_DEBUG_OVERKILL) 
    logit("", "wsMergeMenuIntoPSCNLList(%u): after malloc\n", pEnv);

  if(!pTemp)
  {
    ret = WS_ERR_MEMORY;
    logit("t", "wsMergeMenuIntoPSCNLList: memory allocation error for %d bytes\n",
  ...
  for(j=0;i<SCNLsFound && j<pEnv->iNumPSCNLs;j++)
  {
          ...
          < 0)
    {
      ...
      if(WS_CL_DEBUG & WS_DEBUG_OVERKILL) 
        logit("", "wsMergeMenuIntoPSCNLList: Adding (%s:%s:%s:%s)- %s:%s to PSCNLList\n",
              SCNLReplyList[i].sta, SCNLReplyList[i].chan, SCNLReplyList[i].net, SCNLReplyList[i].loc,
    ...
    else if(rcComp == 0)
    {
      ...
      ****************************************************/
      ...
      rc = wsCombinePSCNLs(&(pTemp[iNumCurrentSCNLs]), &(pEnv->pPSCNLBuffer[j]), &(SCNLReplyList[i]));
...
int wsCombinePSCNLs(WS_PSCNL pResult, WS_PSCNL pIn1, WS_PSCNL pIn2)
{
           ...
           k < (sizeof(pResult->MenuList)/sizeof(WS_MENU)); k++)
  {
    ...
    else if(pIn1->MenuList[i]->menunum > pIn2->MenuList[j]->menunum)
    {
      ...
      if(WS_CL_DEBUG & WS_DEBUG_OVERKILL) 
        logit("", "wsCombinePSCNLs: Adding %s:%s as menu[%d] to chan (%s:%s:%s)\n",
  ...
  else if(j< pIn2->iNumMenus)  
  {
    ...
    for(ctr=j; ctr < j+ iRemainingMenusToCopy; ctr++)
    {
      ...
      if(WS_CL_DEBUG & WS_DEBUG_OVERKILL) 
        logit("", "wsCombinePSCNLs(rem): Adding %s:%s as menu[%d] to chan (%s:%s:%s)\n",
...
int wsGetTraceFromServer(TRACE_REQ* getThis, WS_MENU menu, int iTraceType, int timeout_msec)
  ...
  {
    sprintf( request, "GETSCNL: %d %s %s %s %s %lf %lf %d\n",
            menu_reqid++, getThis->sta, getThis->chan, getThis->net, getThis->loc,
...
*            WS_WRN_FLAGGED: server sent us a no-data flag
  */
  ...
  getThis->actLen = strlen( reply ) - scnl_pos;
...
int wsSearchSCNL( TRACE_REQ* getThis, WS_MENU* menup, wsHANDLE pEnv)
{
  ...
  if(menu_start)
  {
    ...
    {
      if(WS_CL_DEBUG & WS_DEBUG_DATA_WARNINGS) 
        logit("t","wsSearchSCNL(): WARNING additional menu for (%s,%s,%s,%s) not found, all %d menus already processed.\n", 
...
int wsGetServerPSCNL( char* addr, char* port, WS_PSCNL* pscnlp, wsHANDLE pEnv )
{
  ...
  if(!(pFirstPSCNL = malloc(sizeof(WS_PSCNL_REC))))
  {
    logit("t","wsGetServerPSCNL(): Error! Failed to allocate %d bytes.\n",
      sizeof(WS_PSCNL_REC));
    return(WS_ERR_MEMORY);
  }
  ...
  for(i=0; i < pEnv->iNumPSCNLs; i++)
  {
    for(j=0; j < pEnv->pPSCNLBuffer[i].iNumMenus; j++)
    {
      ...
      {
        if(!(pNewPSCNL = malloc(sizeof(WS_PSCNL_REC))))
        {
          logit("t","wsGetServerPSCNL(): Error! Failed to allocate %d bytes.\n",
                sizeof(WS_PSCNL_REC));
          return(WS_ERR_MEMORY);
