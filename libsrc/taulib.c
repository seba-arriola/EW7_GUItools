...
These functions have been converted to C at the WC*ATWC.  These are used 
...
/*
 $$$$$ calls no other routines $$$$$
   ...
   logical unit lu.
*/
void bkin (FILE *hFileTbl, int nrec, int len, double buf[], FILE *hFile)
{
static	int	i;
...
if (nrec > 0)
    {
    if (DEBUGL) fprintf (hFile, "bkin read; nrec=%ld, reclen=%ld, len=%ld\n", 
                        nrec, iRecLen, len);
...
for (i=0; i<iNSeg; i++)
    ...
    if (DEBUGL) fprintf (hFile, "j phcd phtmp = %ld %s %s\n", j+1, szPhcd[j],
                        phtmp);
    ...
    nsgpt[kseg] = i;
    if (prflg[2] && DEBUGL) 
        fprintf (hFile, "kseg nsgpt segcd = %ld %ld %s\n",
...
for (i=0; i<no; i++)
    ...
    for (k=0; k<=kseg; k++)
        ...
        bSegmsk[l] = TRUE;
	if (DEBUGL) 
	    fprintf (hFile, "Brnset:  phase found - i k l segcd = %ld %ld %ld %s\n",
                     i, k, l, segcd[k]);
        }
	// If no matching entry is found, warn the caller.
    if (!fnd & DEBUGL) 
        fprintf (hFile, "Brnset:  phase %s not found\n", phlst[i]);
    continue;
    ...
    for (j=j1-1; j<j2; j++)
        for (k=0; k<=kseg; k++)
            if (!strcmp (cmdlst[j], segcd[k]) )
	        {
                ...
                bSegmsk[l] = TRUE;
                if (DEBUGL) 
                    fprintf (hFile, "Brnset:  cmdlst found - j k l segcd = "
                             "%ld %ld %ld %s\n", i, k, l, segcd[k]);
                }
    }
...
for (i=0; i<iNSeg; i++)
    ...
    if (DEBUGL) fprintf (hFile, "Brnset:  Segment pointer (%ld) missing?\n", i);
    continue;
    SkipAhead:
    ...
    j2--;
    if (!fnd & DEBUGL) 
        fprintf (hFile, "Brnset:  the following phases have been selected:\n");
    fnd = TRUE;
    if (DEBUGL)
        {
        fprintf (hFile, "     %ld  ", i+1);
        for (j=j1-1; j<j2; j++) fprintf (hFile, "%10s", segcd[j]);
        fprintf (hFile, "\n");
	}
    }
return;
...
void depcor (int nph, FILE *hFile, FILE *hFileTbl)
{
static	bool	noend, noext;
static	double	dtol = 1.e-6;
static	int	i, iph, is, j, kph, ks, mu, k, ll, m;	// based on 0
static	int	i1, i2, l, ms, n1, k1, k2;		// based on 1
static	int	lpower = 7, lp;
static	double	tol = 0.01;
static	double	tauus1[2], tauus2[2], xus1[2], xus2[2], ttau, tx, sgn, 
        umin, u0, u1, z0, z1, fac, du, ztol;
static	double	*tup;
...
if (DEBUGL) fprintf (hFile, "depcor:  nph iNph0 %ld %ld\n", nph+1, iNph0+1);
if(nph == iNph0) goto ConstructTau;
iNph0 = nph;
dUs[nph] = umod (dZs, &iIsrc[nph], nph);
if (DEBUGL) fprintf (hFile, "call umod, isrc=%ld\n", iIsrc[nph]);
	// If we are in a high slowness zone, find the slowness of the lid.
umin = dUs[nph];
ks = iIsrc[nph];
if (DEBUGL) fprintf (hFile, "ks us %ld %lf \n", ks+1, umin);
for (i=0; i<ks+1; i++)
...
if (DEBUGL) fprintf (hFile, "Source slowness too large.\n");
exit (0);
SkipIt:
k2 = i + 1;
if (DEBUGL) fprintf (hFile, "k2 umin %ld %lf \n", k2, umin);
...
bkin (hFileTbl, iNDex[ks][nph], iKu[nph]+iKm[nph], tup, hFile);
goto MoveDepth;
...
if (DEBUGL) fprintf (hFile, "bkin %ld %lf %lf %lf\n", ks+1, sgn, dTauu[0][nph], 
                    dXu[0][nph]);

	// Fiddle pointers.
Fiddle:	
iMsrc[nph] = ks + 1;
if (DEBUGL) fprintf (hFile, "msrc sgn %ld %lf\n", iMsrc[nph], sgn);
noend = FALSE;
...
k1 = k2 - 1;
if (noend) k1 = k2;
if (DEBUGL) fprintf (hFile, "noend noext k2 k1 %ld %ld %ld %ld\n", 
                    noend, noext, k2, k1);
if (noext)
    {
    ...
    for (k=0; k<k1; k++)
	    ...
	    {
            dXc[mu] = dXu[mu][nph];
	    if (DEBUGL) fprintf (hFile, "up x:  k mu %ld %ld %lf %lf\n", 
	                        k+1, mu+1, dXu[mu][nph], dXc[mu]);
            mu++;
	    }
	}
    goto CalcInt;
...
mu = 0;
if (DEBUGL) fprintf (hFile, "u0 z0 %lf %lf\n", u0, z0);
if (DEBUGL) fprintf (hFile, "u1 z1 %lf %lf\n", u1, z1);
for (k=0; k<k1; k++)
    {
    tauint (dPu[k][nph], u0, u1, z0, z1, &ttau, &tx, hFile);
    dTauc[k] = dTauu[k][nph] + sgn*ttau;
    if (fabs (dPu[k][nph]-dPux[mu][nph]) <= dtol)
        {
        dXc[mu] = dXu[mu][nph] + sgn*tx;
        if (DEBUGL) 
            fprintf (hFile, "up x:  k mu %ld %ld %lf %lf\n", k+1, mu+1,
                     dXu[mu][nph], dXc[mu]);
        mu++;
        }
    }
...
xus2[nph] = 0.;
mu--;
if (fabs (umin-dUs[nph]) > dtol && fabs (umin-dPux[mu][nph]) <= dtol) mu--;
...
if (fabs (dPux[mu][nph]-umin) <= dtol && fabs (dUs[nph]-umin) <= dtol)
    ...
    xus1[nph] = dXc[mu];
    if (DEBUGL) 
        fprintf (hFile, "is ks tauus1 xus1 %ld %ld %lf %lf  *\n", is+1, ks+1, 
                 tauus1[nph], xus1[nph]);
    goto TurnInt;
    }
...
if (fabs (dZm[is][nph]-dZs) > dtol)
	...
	// integral is needed.
    tauint (umin, dPm[is][nph], dUs[nph], dZm[is][nph], dZs, &ttau, &tx, hFile);
    tauus1[nph] += ttau;
    xus1[nph] += tx;
    if (DEBUGL) 
        fprintf (hFile, "is ks tauus1 xus1 %ld %ld %lf %lf\n", is+1, ks+1, 
...
for (i=is+1; i<iMt[nph]; i++)
    ...
    z1 = dZm[i][nph];
    if (u1 < umin) break;
    tauint (umin, u0, u1, z0, z1, &ttau, &tx, hFile);
    tauus2[nph] += ttau;
    xus2[nph] += tx;
    }
if (DEBUGL) fprintf (hFile, "is ks tauus2 xus2 %ld %ld %lf %lf\n", is+1, ks+1, 
                    tauus2[nph], xus2[nph]);
...
tauint (umin, u0, umin, z0, z1, &ttau, &tx, hFile);
tauus2[nph] += ttau;
xus2[nph] += tx;
if (DEBUGL) 
    fprintf (hFile, "is ks tauus2 xus2 %ld %ld %lf %lf\n", is+1, ks+1, 
             tauus2[nph], xus2[nph]);
...
if (nph == 1)
	...
	// would be needed for sPg and SPg when the source is in the deep 
	// mantle.

    for (j=0; j<iNBrn; j++)
        {
        if ((!strncmp (szPhcd[j], "sP", 2) || !strncmp (szPhcd[j], "SP", 2)) &&
             dPx[j][1] > 0.)
	    {
            if (DEBUGL) 
                fprintf (hFile, "Depcor:  j phcd px umin = %ld  %s %lf %lf %lf\n",
                         j+1, szPhcd[j], dPx[j][0], dPx[j][1], umin);
    ...
    for (j=0; j<iNBrn; j++)
        {
        if ((!strncmp (szPhcd[j], "pS", 2) || !strncmp (szPhcd[j], "PS", 2)) &&
            dPx[j][1] > 0.)
	    {
            if (DEBUGL) 
                fprintf (hFile, "Depcor:  j phcd px umin = %ld  %s %lf %lf %lf\n", 
...
Integral:
j = 0;
if (DEBUGL) fprintf (hFile, "Depcor:  do pS or sP integral - iph = %ld\n", iph+1);
for (i=1; i<iMt[iph]; i++)
    {
...
if (fabs (dZm[j][iph]-z1) <= dtol) goto PastPart;
	// Unless the turning point is right on a sample slowness, one more
	// partial integral is needed.
tauint (umin, dPm[j][iph], umin, dZm[j][iph], z1, &ttau, &tx, hFile);
tauus1[iph] += ttau;
xus1[iph] += tx;
if (DEBUGL) fprintf (hFile, "is ks tauusp xusp %ld %ld %lf %lf\n", j+1, ks+1, 
                    tauus1[iph], xus1[iph]);
    
PastPart:
dUa[0][nph] = -1.;
// if(dODep >= dDepLim || dODep <= .1) go to 43
...
if (DEBUGL) 
    fprintf (hFile, "Add:  nph is ka odep du us = %ld %ld %ld %lf %lf %lf\n", 
             nph+1, is+1, iKa, dODep, du, dUs[nph]);
lp = lpower;
k = -1;
for (l=iKa; l>=1; l--) 
    {
	...
	for (i=1; i<is+1; i++)
	    {
            tauint (dUa[k][nph], dPm[j][nph], dPm[i][nph], dZm[j][nph], 
                    dZm[i][nph], &ttau, &tx, hFile);
            dTaua[k][nph] += ttau;
            j = i;
	    }
        if (DEBUGL) fprintf (hFile, "l k ua taua %ld %ld %lf %lf\n", l, k+1, 
                            dUa[k][nph], dTaua[k][nph]);
        }
    if (fabs (dZm[is][nph]-dZs) > dtol) 
        {
	// Unless the source is right on a sample slowness, one more partial
...
Construct:   
j = 0;
if (DEBUGL) fprintf (hFile, "mu %ld\n", mu+1);
if (DEBUGL) fprintf (hFile, "killer loop:\n");
for (i=0; i<iNSeg; i++)
    {
    if (!bSegmsk[i]) continue;
    if (DEBUGL) fprintf (hFile, "i iidx nafl nph %ld %ld %ld %ld\n", i+1, 
                        iIdx[i], iNafl[i][0], nph+1);
    ...
    i1 = iIndx[i][0];
    i2 = iIndx[i][1];
    if (DEBUGL) fprintf (hFile, "i1 i2 sgn iph %ld %ld %lf %ld\n", i1, i2, sgn, 
                        iph+1);
    m = 0;
    for (k=i1-1; k<i2; k++)
        {
    ...
    k = i2 - 1;
    if (DEBUGL) fprintf (hFile, "k m %ld %ld\n", k+1, m+1);
    goto SkipALeg;
    NextLeg:
    if (DEBUGL) fprintf (hFile, "k m %ld %ld\n", k+1, m+1);
    ...
    dPt[k] = umin;
    fac = dFcs[i][0];
    if (DEBUGL) fprintf (hFile, "ki fac %ld %lf\n", iKi, fac);
    dTau[k][0] = fac * (tauus1[iph]+tauus2[iph]+tauus1[kph]+tauus2[kph]) +
                 sgn * tauus1[nph];
    if (DEBUGL) 
        fprintf (hFile, "&&&&& nph iph kph tauus1 tauus2 tau %ld %ld %ld %lf %lf"
                 " %lf %lf %lf\n", nph+1, iph+1, kph+1, tauus1[0], tauus1[1], 
                 tauus2[0], tauus2[1], dTau[k][0]);
    SkipALeg:
    m = 0;
    ...
    if (DEBUGL) fprintf (hFile, "j jndx jidx %ld %ld %ld %ld  %s\n", j+1,
                        iJNDex[j][0], iJNDex[j][1], iJIDex[j], szPhcd[j]);
    for (ll=0; ll<2; ll++)
        {
        while (fabs (dPux[m][nph]-dPx[j][ll]) > dtol)
	    {
            if (m >= mu)
	        {
                dXBrn[j][ll] = fac * (xus1[iph]+xus2[iph]+xus1[kph]+xus2[kph]) +
                               sgn*xus1[nph];
                if (DEBUGL) fprintf (hFile, "x up:  j l end %ld %ld\n", 
                                    j+1, ll+1);
                if (DEBUGL) fprintf (hFile, "&&&&& nph iph kph xus1 xus2 xbrn %ld %ld %ld "
                                   "%lf %lf %lf %lf %lf\n", nph+1, iph+1, kph+1, 
                                   xus1[0], xus1[1], xus2[0], xus2[1], 
                                   dXBrn[j][ll]);
                goto EndInnerLoop;
		}
            m++;
        ...
        if (DEBUGL) fprintf (hFile, "x up:  j l m %ld %ld %ld\n", j+1, ll+1,m+1);
        EndInnerLoop:;
        }
    if(j+1 < iNBrn) 
        {
...
void depset (double dep, double usrc[], FILE *hFile, FILE *hFileTbl)
{
static	bool	dop, dos;
static	int	i, ind, j, k, nph;		// based on 0
static	int	in;				// based on 1
static	double	rdep;
...
if (bPrnt[0] || bPrnt[1]) fprintf (hFile, "Depth = %lf\n", dep);

CallDepcor:
if (iNph0 <= 0)
    {
    if (dop) depcor (0, hFile, hFileTbl);
    if (dos) depcor (1, hFile, hFileTbl);
    }
  else
    {
    if (dos) depcor (1, hFile, hFileTbl);
...
for (i=0; i<iNSeg; i++)
    ...
    if (!bSegmsk[i]) continue;
    nph = abs (iNafl[i][0]) - 1;
//  printf ("i iidx nph msrc nafl = %ld %ld %ld %ld %ld\n", i, iIdx[i], nph,
//           iMsrc[nph], iNafl[i][0]);
    ...
    iDelSet:
    iDel[j][2] = iNafl[i][0];
//  printf ("spfit:  j in = %ld %ld\n", j, in );
    spfit (j, in, hFile);
    iMbr1 = min (iMbr1, j+1);
    iMbr2 = max (iMbr2, j+1);
    if (j+1 >= iNBrn) continue;
    j++;
//  printf ("j jidx indx jndx = %ld %ld %ld %ld\n", j, iJIDex[j], iIndx[i][1],
//          iJNDex[j][1]);
    if (iJIDex[j] <= iIndx[i][1] && iJNDex[j][1] > 0) goto iDelSet;
    }
if (DEBUGL)
    {
    fprintf (hFile, "mbr1 mbr2 %ld %ld\n", iMbr1, iMbr2);
    fprintf (hFile, "msrc isrc odep zs us %ld %ld %ld %ld %lf %lf %lf %lf\n", 
     iMsrc[0], iMsrc[1], iIsrc[0]+1, iIsrc[1]+1, dODep, dZs, dUs[0], dUs[1]);
    fprintf (hFile, "         %ld\n", iKi);
    for (i=0; i<iNSeg; i++)
        fprintf (hFile, "%ld %ld %ld %lf\n", i+1, iIdx[i], ikk[i]+1, dPk[i]);
    }
usrc[0] = dUs[0] / dPn;
usrc[1] = dUs[1] / dPn;
return;
...
void findtt (int jb, double x0[], int *n, double tt[], double dtdd[], 
             ...
             FILE *hFile)
{
static	int	i, in, ij, is, ie, jj, j, ln, nph;	// based on 0
static	int	le;		       		 	// based on 1
static	double	x, p0, p1, arg, dp, dps, delp, tol=3.e-6, ps, deps=1.e-10,
...
for (ij=iDel[jb][0]-1; ij<iDel[jb][1]; ij++)
    ...
    for (i=is; i<ie+1; i++)
	    ...
	    strcpy (phnm[*n-1], szPhcd[jb]);
            in = strstr (phnm[*n-1], "ab") - phnm[*n-1];
	    ...
	    if (jj == 0)
	        {
                ...
                if (arg < 0.)
		    if (DEBUGL) fprintf (hFile, "Bad sqrt argument %lf\n", arg);
	...
	if (DEBUGL) fprintf (hFile, "Failed to find phase:  %s %lf %lf %lf %lf %lf\n",
...
SkipABit:
    ...
    *n = *n + 1;
    ...
    ln = strchr (szPhcd[jb], ' ') - szPhcd[jb] - 1;
    if (ln <= -1) ln = strlen (szPhcd[jb]);
...
return;
SayWarning:
if (DEBUGL) fprintf (hFile, "More than %ld arrivals found.\n", MAX_PHASES);
return;
}
...
void fitspl (int i1, int i2, double tau2[JOUT][4], double x1, double xn2, 
             double coef2[5][JOUT])
{
static	double	a[2][60], ap[3], b[60], alr, g1, gn;
static	int	i, ie, is, j, n, n1;
...
int iupcor (PSZ phnm, double dtdd, double *xcor, double *tcor)
{
static	double	x, dp, dps, ps;
static	double	oldep = -1.;
static	int	is, ie;		// based on 1
static	int	jp = -1;	// based on 0
static	int	js = -1;	// based on 0
static	int	i, j, jb;	// based on 0
static	int	iup;

iup = 1;
// printf ("oldep odep %lf %lf\n', oldep, dODep);
if (oldep != dODep)
    {
    oldep = dODep;
	// Find the upgoing P branch.
//  printf ("mbr1 mbr2 %ld %ld\n", iMbr1, iMbr2);
    for (jp=iMbr1-1; jp<iMbr2; jp++)
        {
//      printf ("jp phcd xbrn %ld %s %lf\n", jp, szPhcd[jp], dXBrn[jp][0);
        if ((!strcmp (szPhcd[jp], "Pg") || !strcmp (szPhcd[jp], "Pb") || 
             !strcmp (szPhcd[jp], "Pn") || !strcmp (szPhcd[jp], "P")) && 
             dXBrn[jp][0] <= 0.) goto JumpAhead;
        }
    jp = -1;
	// Find the upgoing S branch.
JumpAhead:
    for (js=iMbr1-1; js<iMbr2; js++)
        {
//      printf ("js phcd xbrn %ld\n", js, szPhcd[js], dXBrn[js][0]);
        if((!strcmp (szPhcd[js], "Sg") || !strcmp (szPhcd[js], "Sb") || 
            !strcmp (szPhcd[js], "Sn") || !strcmp (szPhcd[js], "S")) && 
            dXBrn[js][0] <= 0.) goto JumpAhead2;
        }
    js = -1;
    }
JumpAhead2:
...
// printf ("jp js %ld %ld\n", jp, js);
if (!strncmp (phnm, "P", 1) || !strncmp (phnm, "p", 1)) 
    {
    jb = jp;
    if (jb <= 0) goto SetVar;
    }
    ...
    {
    if(strncmp (phnm, "S", 1) && !strncmp (phnm, "s", 1)) goto iUpcor;
    jb = js;
    if (jb <= 0) goto SetVar;
    }
is = iJNDex[jb][0] + 1;
ie = iJNDex[jb][1];
ps = fabs (dtdd) / dDn;
// printf ("jb is ie dtdd dn ps %ld %ld %ld %lf %lf %lf\n", jb, is, ie, dtdd, 
//         dDn, ps);
if (ps < dPt[is-1] || ps > dPt[ie]) goto iUpcor;
for (i=is-1; i<ie; i++)
    {
//  printf ("i dPt %ld %lf\n", i, dPt[i]);
    if (ps <= dPt[i]) goto JumpAhead3;
    }
goto iUpcor;
JumpAhead3:
j = i - 1;
dp = dPt[ie-1] - ps;
dps = sqrt (fabs (dp));
x = dTau[j][1] + 2.*dp*dTau[j][2] + 1.5*dps*dTau[j][3];
// printf ("j pt dp dps x %ld %lf %lf %lf %lf\n", j, dPt[ie-1], dp, dps, x);
*tcor = dTn * (dTau[j][0]+dp*(dTau[j][1]+dp*dTau[j][2]+dps*dTau[j][3])+ps*x);
*xcor = DEG * x;
// printf ("iupcor xcor tcor %ld %lf %lf\n", iup, *xcor, *tcor);
return (iup);
...
void pdecu (int i1, int *i2, double x0, double x1, double xmin, int in, 
            FILE *hFile)
{
static	double	dx, dx2, sgn, rnd, xm, axm, x, h1, h2, hh, xs;
static	int	i, j, k, m;		// based on 0
static	int	is, n, ie;		// based on 1

if (DEBUGL) fprintf (hFile, "Pdecu:  ua = %lf\n", dUa[0][in]);
if (dUa[0][in] > 0.)
    {
    if (DEBUGL) fprintf (hFile, "Pdecu:  fill in new grid\n");
    k = i1;
    for (i=0; i<iKa; i++)
        {
        dPt[k] = dUa[i][in];
    ...
    *i2 = k + 1;
    if (DEBUGL)
        {
        fprintf (hFile, " %ld %lf\n", i, dPt[i]);
        for (i=i1-1; i<*i2; i++) fprintf (hFile, "%ld ", dTau[i][0]);
        fprintf (hFile, "\n");
	}
    return;
    }
...
{
static	int	i, ij, it, ib, j, k, kkk, l, m;		// Indices based on 0
static	int	il[10], iu[10];				// "      "       "
static	double	tmpkey, r;
...
void spfit (int jb, int in, FILE *hFile)
{
static	double	dmn, pmn, dmx, hm, shm, thm, p0, p1, tau0, tau1, x0, x1, pe;
static	double	pe0, spe0, scpe0, pe1, spe1, scpe1, dpe, dtau;
static	double	dbrnch = 2.5307274;
static	double	xmin = 3.92403e-3;
static	double	dtol = 1.e-6;
static	double	ptol = 2.e-6;
static	char	szDisc[4];
static	bool	newgrd, makgrd;
static	int	i, j, k;		    			// based on 0
static	int	i1, i2, nn, is, mxcnt, mncnt;			// based on 1

if (bPrnt[0]) fprintf (hFile, "\n");
i1 = iJNDex[jb][0];
i2 = iJNDex[jb][1];
if (DEBUGL) 
    fprintf (hFile, "Spfit:  jb i1 i2 pt = %ld %ld %ld %lf %lf\n", jb, i1, i2, 
             dPt[i1-1], dPt[i2-1]);
...
if (DEBUGL) 
    fprintf (hFile, "Spfit:  px newgrd = %lf %ld\n", dPx[jb][1], newgrd);
if (!newgrd) 
    ...
    if (DEBUGL) fprintf (hFile, "Spfit:  in k int0 makgrd = %ld %ld %ld %ld\n", 
                        in, k, iInt0[k], makgrd);
    if (in <= 2)
        {
        xmin = dXn * min (max (2.*dODep, 2.), 25.);
        if (DEBUGL) fprintf (hFile, "Spfit:  xmin = %lf %lf\n", xmin, xmin/dXn);
        pdecu (i1, &i2, dXBrn[jb][0], dXBrn[jb][1], xmin, in-1, hFile);
        iJNDex[jb][1] = i2;
	}
    nn = i2 - i1 + 1;
    ...
    if (DEBUGL) 
        {
        fprintf (hFile, "%ld %ld %ld %ld %ld %ld %lf %lf\n", jb, k, nn, in, 
                 newgrd, makgrd, dXBrn[jb][0], dXBrn[jb][1]);
        for (i=0; i<nn; i++)
            fprintf (hFile, "%ld %lf %lf %lf %lf %lf %lf %lf\n", i, dPt[i-1+i1],
                     dTau[i-1+i1][0], dTCoef[k][0][i], dTCoef[k][1][i], 
                     dTCoef[k][2][i], dTCoef[k][3][i], dTCoef[k][4][i]);
	}
...
if (DEBUGL) 
    for (i=i1-1; i<i2; i++)
       fprintf (hFile, "%ld %lf %lf %lf %lf\n", i, dPt[i],
                dTau[i][0], dTau[i][1], dTau[i][2]);
pe = dPt[i2-1];
p1 = dPt[i1-1];
tau1 = dTau[i1-1][0];
x1 = dTau[i1-1][1];
...
for (i=is-1; i<i2; i++)
    ...
    DownThere:
    if (bPrnt[0]) 
        fprintf (hFile, "%s %ld %lf %lf %lf %lf %lf %lf %lf\n", szDisc, j, 
                 dPt[j], dTau[j][0], dTau[j][1], dTau[j][2], dTau[j][3],
                 DEG*dXLim[0][j], DEG*dXLim[1][j]);
    dmx = max (dmx, dXLim[1][j]);
    j = i;
    }
if (bPrnt[0] && DEBUGL) fprintf (hFile, "%ld %lf\n", j, dPt[j]);
dXBrn[jb][0] = dmn;
dXBrn[jb][1] = dmx;
dXBrn[jb][2] = pmn;
iDel[jb][0] = 1;
...
if (bPrnt[0] && bPrnt[1]) fprintf (hFile, "\n");
if(dDBrn[jb][0] > 0.)
    {
    dDBrn[jb][0] = dmx;
    dDBrn[jb][1] = dbrnch;
    if (bPrnt[1]) 
        fprintf (hFile, " %s %ld %ld %lf %lf %lf %lf %lf %ld %ld %ld %ld %ld %ld\n",
             szPhcd[jb], iJNDex[jb][0], iJNDex[jb][1], DEG*dXBrn[jb][0], 
             DEG*dXBrn[jb][1], dXBrn[jb][2], DEG*dDBrn[jb][0], DEG*dDBrn[jb][1],
             iDel[jb][0], iDel[jb][1], iDel[jb][2], in, newgrd, makgrd);
    }
  else
    {
    if (bPrnt[1]) 
        fprintf (hFile, " %s %ld %ld %lf %lf %lf         %ld %ld %ld %ld %ld %ld\n",
             szPhcd[jb], iJNDex[jb][0], iJNDex[jb][1], DEG*dXBrn[jb][0], 
             DEG*dXBrn[jb][1], dXBrn[jb][2], 
             iDel[jb][0], iDel[jb][1], iDel[jb][2], in, newgrd, makgrd);
    }
if (mxcnt > mncnt || mncnt > mxcnt+1) 
    if (DEBUGL) fprintf (hFile, "Bad interpolation on %s\n", szPhcd[jb]);
return;
}

void tabin (FILE * *hFileTbl, PSZ modnam, FILE *hFile)
{
static	FILE	*hFileHed;
static	int	i, j, k, nph, ind, l;			// Based on 0
static	int	nl, len2;
static	char	phdif[6][PHASE_LENGTH];
static	char	szFile[32];
...
if ((hFileHed = fopen (szFile, "rb")) == NULL)
    {
    if (DEBUGL) fprintf (hFile, "Header file not found\n");
    exit (0);
    }
...
if ((*hFileTbl = fopen (szFile, "rb")) == NULL)
    {
    if (DEBUGL) fprintf (hFile, "Table file not found\n");
    exit (0);
    }
...
if (DEBUGL) 
    {
    fprintf (hFile, "nasgr nl len2 %ld %ld %ld\n", iRecLen, nl, len2);
    fprintf (hFile, "iNSeg iNBrn iMt ku km %ld %ld %ld %ld %ld %ld %ld %ld\n", 
             iNSeg, iNBrn, iMt[0], iMt[1], iKu[0], iKu[1], iKm[0], iKm[1]);
    fprintf (hFile, "%lf %lf %lf\n", dXn, dPn, dTn);
    for (i=0; i<iMt[1]; i++)
        fprintf (hFile, "%ld %ld %lf %lf %ld %lf %lf\n", i+1, iNDex[i][0], 
                 dPm[i][0], dZm[i][0], iNDex[i][1], dPm[i][1], dZm[i][1]);
    for (i=0; i<iKu[1]+1; i++)
        fprintf (hFile, "%ld %lf %lf\n", i+1, dPu[i][0], dPu[i][1]);
    for (i=0; i<iKm[1]; i++)
        fprintf (hFile, "%ld %lf %lf\n", i+1, dPux[i][0], dPux[i][1]);
    for (i=0; i<iNSeg; i++)
        fprintf (hFile, "%ld %5ld %5ld %5ld %5ld %5ld %5ld %5ld %lf %lf %lf\n", 
                 i+1, iNafl[i][0], iNafl[i][1], iNafl[i][2], iIndx[i][0], 
                 iIndx[i][1], iKndx[i][0], iKndx[i][1], dFcs[i][0], dFcs[i][1], 
                 dFcs[i][2]);
    for (i=0; i<iNBrn; i++)
        fprintf (hFile, "%ld %ld %ld %lf %lf %lf %lf %s\n", i+1, iJNDex[i][0], 
...
void tauint (double ptk, double ptj, double pti, double zj, double zi, 
             double *tau2, double *x, FILE *hFile)
{
static	char	msg[72];
static	double	xx, b, sqk, sqi, sqj, sqb, tmp;
...
if (b*b < sqk) 
    ...
    xx = asin (max (min ((b*pti-sqk)/(ptj*fabs (pti-b)), 1.e0), -1.e0)) - tmp;
    *x = -ptj*xx/sqb;
    }
  else
    {
...
{
static	double	ali, alr, b3h, b1h, bih, th0p, th2p, th3p, th2m;
static	double	del[5], sdel[5], deli[5], d3h[4], d1h[4], dih[4], d[4];
static	int	i, j, k, m;			// based on 0
static	int	n2, is, l;			// based on 1 (i1, i2 also)
...
for (i=0; i<n2; i++)
    ...
    coef2[0][l-1] = (0.125*del[4]*sdel[4] - (0.75*sdel[4] + 0.375*deli[4]*del[3]
                   - sdel[3])*del[3])*d[3];
...
for (j=0; j<4; j++)
	...
	// Construct the contributions G;i-1(p;i-2) and G;i-1(p;i).
SkipSome:
    if (j <= 1) coef2[0][l-1] = (0.125*del[2]*sdel[2] - (0.75*sdel[2] + 0.375*
                 deli[2]*del[3] - sdel[3]) * del[3]) * d[2] - (0.125*d3h[1] - 
                 (0.75*d1h[1] + 0.375*dih[1]*del[3]) * del[3]) * d[1] - 
                 (0.125*d3h[0] - (0.75*d1h[0] + 0.375*dih[0]*del[3]) * del[3]) *
                 d[0];
    if (l-i1 > 1) coef2[1][l-3] = (0.125*del[0]*sdel[0] - (0.75*sdel[0] +
                   0.375*deli[0]*del[1] - sdel[1]) * del[1]) * d[0];
	// Construct the contributions -dG;i-1(p)/dp | p;i-2, p;i-1, and p;i.
    if (j <= 1) coef2[2][l-1] = -0.75 * ((sdel[2] + deli[2]*del[3] -
                 2.*sdel[3]) * d[2] - (d1h[1] + dih[1]*del[3]) * d[1] - (d1h[0]+
                 dih[0]*del[3]) * d[0]);
...
for (j=iMbr1-1; j<iMbr2; j++)
    if (iJNDex[j][1] > 0) findtt (j, x, n, &tmp[0][0], &tmp[1][0], &tmp[2][0],
                                  &tmp[3][0], ctmp, hFile);
if (*n-1 < 0) return;
if (*n-1 == 0)
    iptr[0] = 0;
  else
    r4sort (*n, &tmp[0][0], iptr);
k = -1;
for (i=0; i<*n; i++)
    {
    j = iptr[i];
...
{
static	double	dep;
static	int	i, j;			// Index based on 0
static	double	dtol = 1.e-6;

for (i=1; i<iMt[nph]; i++) if (dZm[i][nph] <= zs) goto ItsOK;
dep = (1. - exp (zs)) / dXn;
// printf ("Source depth ('%6.1lf') too deep.", dep);
exit (0);
ItsOK:
if (fabs (zs-dZm[i][nph]) > dtol || fabs (dZm[i][nph]-dZm[i+1][nph]) > dtol) 
    {
    j = i - 1;
    *isrc = j;
