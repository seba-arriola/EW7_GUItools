...
char *GetOpAgency( char *pszNetID )
{
   if ( !strcmp( pszNetID, "AT" ) )
       ...
       *                                                                *
       *  Arguments:                                                    *
       *   pPStruct         Incoming Ppick structure                    *
       ...
       *                                                                *
       ******************************************************************/
int GetPhaseTime( PPICK *pPStruct, HYPO *pHypo, PHASE *pPhase, int iPhaseToGet )
{
...
/* Figure distance to station from this hypocenter */
   azidelt = GetDistanceAz( (LATLON *) pHypo, (LATLON *) pPStruct );
   if ( (hFile = fopen( "taulib1.lis", "w" )) == NULL )
   {
      logit( "t", "taulib.lis not opened in GetPhase\n" );
      return( 0 );
   }
...
/* Query taulib for phases */
   ...
   tabin( &hFileTbl, szModnam, hFile );  /* Read in iaspei91 tau/p file */
   brnset( iNo, szPhaseLst, iPrint, hFile );  /* Set up phases desired */
   depset( pHypo->dDepth, dUsrC, hFile, hFileTbl ); /* Depth correction */
/* Compute phase times */
   trtm( azidelt.dDelta, &n, dTT, dDTdd, dDTdh, dDDdp, szPhase, hFile );
   pPhase->iNumPhases = n;               
   for ( j=0; j<n; j++ )                 /* Fill up PHASE array */
   {         /* Odd way to do elevation correction but it works */
      pPhase->dPhaseTT[j] = dTT[j] + pPStruct->dElevation/EARTHRAD;
      pPhase->dPhaseTime[j] = dTT[j] + pHypo->dOriginTime +
                            pPStruct->dElevation/EARTHRAD;
...
/* Get the region */
   if ( dLatT >= 53. && dLatT < 53.5 && dLonT >= 174. && dLonT <= 188. )
      ...
      iMsgRegion = 0;                 /* U.S. West Coast */
   else if ( dLatT >= 32.0 && dLatT <= 34.0 && dLonT >= 231. && dLonT < 248. )
      iMsgRegion = 0;                 /* U.S. West Coast */
      ...
      iMsgRegion = 12;                /* Eastern Canada */
   else if ( dLatT >= 17.0 && dLatT < 20.0 && dLonT >= 291. && dLonT < 296.5 )
