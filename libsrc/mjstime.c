...
int DateToDay( SYSTEMTIME *pst )
{
   ...
   return iDay;
       ...
       *                                                                *
       ******************************************************************/
double DateToModJulianSec( SYSTEMTIME stDate )
{
   static double dJulianSec;	/* Modified Julian Second (return value) */
   static int    iLeapDays;	/* Number of leap days since 1858 */
   static int    iYear;         /* Number of years since 1861 */
   static long   lDays;		/* Day-of-year of time to convert */

   iYear = stDate.wYear - 1861;
   iLeapDays = iYear / 4 - 1;
   lDays = (long) DateToDay( &stDate );
   iYear = stDate.wYear - 1858;
   lDays += (long) (iYear*365 + iLeapDays - 321);
   dJulianSec = (double) lDays * 86400.0;
   dJulianSec += (double) stDate.wHour * 3600.0;
   dJulianSec += (double) stDate.wMinute * 60.0;
   dJulianSec += (double) stDate.wSecond;
   dJulianSec += (double) stDate.wMilliseconds/1000. + 0.000001;
   return dJulianSec;
}
